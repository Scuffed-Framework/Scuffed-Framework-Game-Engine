...
//  2025-11-06: Lower minimum requirement to GLFW 3.0. Though a recent version e.g GLFW 3.4 is
//  highly recommended. 2025-09-18: Call platform_io.ClearPlatformHandlers() on shutdown.
...
LRESULT CALLBACK ImGui_Clean_WndProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp,
                                     GLFWwindow *window, WNDPROC originalProc)
{
    // Get backend data for this window
    ImGui_ImplGlfw_Data *bd = ImGui_ImplGlfw_GetBackendData(window);
    if (!bd)
        return CallWindowProcW(originalProc, hwnd, msg, wp, lp);

    ImGuiIO &io = ImGui::GetIO(bd->Context);

    // Handle mouse source detection
    switch (msg)
    {
    case WM_MOUSEMOVE:
    case WM_NCMOUSEMOVE:
    ...
    // Chain to GLFW's original handler
    return CallWindowProcW(originalProc, hwnd, msg, wp, lp);
}
#endif
                                ...
                                GlfwClientApi client_api)
{
    ...
    // ========== THE CLEAN WAY - USE OUR HOOK ==========
#ifdef _WIN32
    GLFW_HookWndProc(window, ImGui_Clean_WndProc);
#endif
