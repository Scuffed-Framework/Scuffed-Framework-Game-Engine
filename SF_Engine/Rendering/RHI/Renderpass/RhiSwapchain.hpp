...
namespace SF::Engine
{
    ...
    {
    public:
        RhiSwapchain(const PhysicalDevice &physicalDevice, const Surface &surface, const LogicalDevice &logicalDevice,
                     const VkExtent2D &extent, const RhiSwapchain *oldSwapchain = nullptr);
        ~RhiSwapchain();

        /**
         * Acquires the next image in the swapchain into the internal acquired image. The function
         * will always wait until the next image has been acquired by setting timeout to UINT64_MAX.
         ...
         * @param waitSemaphore A optional semaphore that is waited on before the image is
         * presented.
