...
namespace SF::Engine
{
        ...
        compositeAlpha(VK_COMPOSITE_ALPHA_OPAQUE_BIT_KHR), activeImageIndex(numeric_limits<uint32_t>::max())
    {
        ...
        {
            this->extent.width  = clamp(this->extent.width, surfaceCapabilities.minImageExtent.width,
                                        surfaceCapabilities.maxImageExtent.width);
            this->extent.height = clamp(this->extent.height, surfaceCapabilities.minImageExtent.height,
        ...
        {
            array<uint32_t, 2> queueFamily            = {RenderSystemFamily, presentFamily};
    ...
    VkResult RhiSwapchain::AcquireNextImage(const VkSemaphore &presentCompleteSemaphore, VkFence fence)
    {
        ...
        auto acquireResult = vkAcquireNextImageKHR(logicalDevice, swapchain, numeric_limits<uint64_t>::max(),
