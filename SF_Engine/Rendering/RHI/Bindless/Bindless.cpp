...
namespace SF::Engine
{
    ...
    void BindlessManager::FreeStorageBuffer(BindlessIndex &index, const std::shared_ptr<Buffer> &fallback)
    {
        if (fallback)
        {
            VkDescriptorBufferInfo bufferInfo{};
    ...
    void BindlessManager::FreeUniformBuffer(BindlessIndex &index, const std::shared_ptr<Buffer> &fallback)
    {
        if (fallback)
        {
            VkDescriptorBufferInfo bufferInfo{};
    ...
    void BindlessManager::Tick()
    {
        ...
        auto it = ranges::remove_if(m_pendingFrees,
