...
#include <Rendering/RHI/Bindless/Bindless.hpp>
#include <cctype>
#include <cstring>
#include <filesystem>
#include <fstream>
...
namespace SF::Engine::Shaders
{
    ...
    namespace
    {
        ...
        // set on failure.
        std::optional<SlangModuleHandle> LoadSlangModule(const ParsedShader &shader, const std::string &resolvedSource,
                                                         std::string &outError)
        {
            ...
            handle.module = handle.session->loadModuleFromSourceString("shader_module",
                                                                       FakeModulePathForLanguage(shader.language),
                                                          ...
                                                          std::string &outError, bool usesBindless)
        {
            ...
            Slang::ComPtr<slang::IBlob> diagnostics;
            SlangResult epResult = module->findAndCheckEntryPoint(entryName.c_str(), ToSlangStage(stage),
                                                                  entry.writeRef(), diagnostics.writeRef());
            ...
            diagnostics = nullptr;
            if (SLANG_FAILED(linkedProgram->getEntryPointCode(0, 0, spirvBlob.writeRef(), diagnostics.writeRef())))
            {
    ...
    std::optional<ParsedShader> ShaderParser::parseSource(const std::string &source, const std::string &name)
    {
        ...
        shader.usesBindless =
                source.find("THIS_IS_A_CERTIFIED_HOOD_CLASSIC_I_MEAN_THE_BINDLESS_IMPL") != std::string::npos;
    ...
    bool ShaderParser::discoverEntryPoints(ParsedShader &shader)
    {
        ...
        for (SlangInt32 i = 0; i < count; i++)
        {
            ...
            if (info.stage == ShaderStage::Compute)
            {
                ...
                info.workgroupSize    = {static_cast<uint32_t>(sizes[0]), static_cast<uint32_t>(sizes[1]),
    ...
    std::optional<std::vector<CompiledShader>>
    ShaderParser::compileAll(const ParsedShader &shader, const std::vector<SF::Engine::Shader::Define> &defines)
    {
        ...
        for (const auto &ep: working.entryPoints)
        {
            ...
            auto spirv = CompileEntryPointFromModule(handle->session.get(), handle->module, ep.name, ep.stage,
                                                     compileError, shader.usesBindless);
            if (spirv.empty())
            {
                setError(compileError);
                std::cerr << "ShaderParser: empty SPIRV for entry '" << ep.name << "' in '" << working.name
                          << "'. Error: " << lastError_ << std::endl;
                return std::nullopt;
            }
    ...
    void ShaderParser::scanPragmas(ParsedShader &shader)
    {
        ...
        while (std::getline(ss, line))
        {
            ...
            } else if (directive == "workgroup_size" && tokens.size() >= 2)
            {
                ...
                shader.workgroupSizeOverride.y = tokens.size() >= 3 ? static_cast<uint32_t>(std::stoul(tokens[2])) : 1u;
                shader.workgroupSizeOverride.z = tokens.size() >= 4 ? static_cast<uint32_t>(std::stoul(tokens[3])) : 1u;
                shader.hasWorkgroupSizeOverride = true;
            } else if ((directive == "multi_compile" || directive == "multi_compile_local") && tokens.size() >= 2)
            {
                                         ...
                                         const std::vector<SF::Engine::Shader::Define> &defines)
    {
        ...
        {
            result += "layout(constant_id = " + std::to_string(sc.constantId) + ") const int " + sc.name + " = " +
