...
namespace SF::Engine
{
    std::shared_ptr<Image2d> Image2d::Create(const std::filesystem::path &filename, VkFilter filter,
    ...
    Image2d::Image2d(std::filesystem::path filename, VkFilter filter, VkSamplerAddressMode addressMode,
        ...
        filename(std::move(filename)), anisotropic(anisotropic), mipmap(mipmap)
    {
        if (load)
        {
            Load();
    ...
    Image2d::Image2d(const UVec2 &extent, VkFormat format, VkImageLayout layout, VkImageUsageFlags usage,
    ...
    void Image2d::Load(std::unique_ptr<Bitmap> loadBitmap)
    {
        ...
        Buffer bufferStaging(loadBitmap->GetLength(), VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VMA_MEMORY_USAGE_CPU_ONLY);
