...
namespace SF::Engine
{
        ...
        EngineRenderpass(stage), params_(params)
    {
        ...
        initialSpectrumTex_ = std::make_unique<Image2dArray>(
                size, 4, VK_FORMAT_R32G32B32A32_SFLOAT, VK_IMAGE_LAYOUT_GENERAL, VK_IMAGE_USAGE_STORAGE_BIT,
                VK_FILTER_LINEAR, VK_SAMPLER_ADDRESS_MODE_REPEAT, false, false);

        spectrumTex_ = std::make_unique<Image2dArray>(size, 8, VK_FORMAT_R32G32B32A32_SFLOAT, VK_IMAGE_LAYOUT_GENERAL,
                                                      VK_IMAGE_USAGE_STORAGE_BIT, VK_FILTER_LINEAR,
                                                      VK_SAMPLER_ADDRESS_MODE_REPEAT, false, false);

        fourierTarget_ = std::make_unique<Image2dArray>(size, 8, VK_FORMAT_R32G32B32A32_SFLOAT, VK_IMAGE_LAYOUT_GENERAL,
                                                        VK_IMAGE_USAGE_STORAGE_BIT, VK_FILTER_LINEAR,
                                                        VK_SAMPLER_ADDRESS_MODE_REPEAT, false, false);

        displacementTex_ =
                std::make_unique<Image2dArray>(size, 4, VK_FORMAT_R32G32B32A32_SFLOAT, VK_IMAGE_LAYOUT_GENERAL,
                                               VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
                                               VK_FILTER_LINEAR, VK_SAMPLER_ADDRESS_MODE_REPEAT, false, true);

        slopeTex_ = std::make_unique<Image2dArray>(size, 4, VK_FORMAT_R32G32_SFLOAT, VK_IMAGE_LAYOUT_GENERAL,
                                                   VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
                                                   VK_FILTER_LINEAR, VK_SAMPLER_ADDRESS_MODE_REPEAT, false, true);

        spectrumParamsBuf_ = std::make_unique<Buffer>(sizeof(SpectrumParameters) * fftSettings_.spectrums.size(),
        ...
        initSpectrumPipeline_ /*****/ = std::make_unique<ComputePipeline>("Shaders/Ocean/OceanFFTSpectrum.shader",
                                                                          kernelDefines("CS_InitializeSpectrum"));
        packConjugatePipeline_ /****/ = std::make_unique<ComputePipeline>("Shaders/Ocean/OceanFFTSpectrum.shader",
                                                                          kernelDefines("CS_PackSpectrumConjugate"));
        updateSpectrumPipeline_ /* */ = std::make_unique<ComputePipeline>("Shaders/Ocean/OceanFFTSpectrum.shader",
                                                                          kernelDefines("CS_UpdateSpectrumForFFT"));
        horizontalFFTPipeline_ /****/ = std::make_unique<ComputePipeline>("Shaders/Ocean/OceanFFTSpectrum.shader",
                                                                          kernelDefines("CS_HorizontalFFT"));
        verticalFFTPipeline_ /******/ = std::make_unique<ComputePipeline>("Shaders/Ocean/OceanFFTSpectrum.shader",
                                                                          kernelDefines("CS_VerticalFFT"));
        assemblePipeline_ /*********/ = std::make_unique<ComputePipeline>("Shaders/Ocean/OceanFFTSpectrum.shader",
                                                                          kernelDefines("CS_AssembleMaps"));
...

        {
            ...
            ImageArrayBarrier(initCmd, initialSpectrumTex_->GetImage(), VK_IMAGE_LAYOUT_GENERAL,
    ...
    void OceanTessellationPipelinePass::setupDescriptorSet()
    {
        ...
        VkDescriptorImageInfo dispInfo{displacementTex_->GetSampler(), displacementTex_->GetView(),
        ...
        VkDescriptorImageInfo slopeInfo{slopeTex_->GetSampler(), slopeTex_->GetView(),
                                        VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL};
    ...
    void OceanTessellationPipelinePass::SetParams(const OceanTessellationParams &params)
    {
        const bool meshChanged =
                (params.patchCount != params_.patchCount) || (params.patchExtent != params_.patchExtent);
    ...
    void OceanTessellationPipelinePass::syncParamsToFrameData()
    {
        ...
        frameData_.sunDirection = glm::length(params_.sunDirection) > 1e-6f ? normalize(params_.sunDirection)
    ...
    void OceanTessellationPipelinePass::RunFFTPass(const CommandBuffer &cmd)
    {
        ...
        ImageArrayBarrier(cmd, displacementTex_->GetImage(), VK_IMAGE_LAYOUT_GENERAL,
                          ...
                          VK_ACCESS_SHADER_READ_BIT, kCompute, VK_PIPELINE_STAGE_TESSELLATION_EVALUATION_SHADER_BIT,
                          /*layerCount=*/4);

        ImageArrayBarrier(cmd, slopeTex_->GetImage(), VK_IMAGE_LAYOUT_GENERAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
                          VK_ACCESS_SHADER_WRITE_BIT, VK_ACCESS_SHADER_READ_BIT, kCompute,
