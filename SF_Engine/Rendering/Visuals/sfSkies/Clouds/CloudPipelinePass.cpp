...
namespace SF::Engine
{
    ...
    CloudPipelinePass::CloudPipelinePass(Pipeline::Stage stage, AtmosphereData &data) : EngineRenderpass(stage), data_(data)
    {
        ...
        cloudRenderRT_ = std::make_unique<Image2d>(quarterRes, VK_FORMAT_R16G16B16A16_SFLOAT, VK_IMAGE_LAYOUT_GENERAL,
                                                   VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_SAMPLED_BIT);
        cloudDepthRT_  = std::make_unique<Image2d>(quarterRes, VK_FORMAT_R32_SFLOAT, VK_IMAGE_LAYOUT_GENERAL,
                                                   VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_SAMPLED_BIT);
        cloudFogRT_    = std::make_unique<Image2d>(quarterRes, VK_FORMAT_R16G16B16A16_SFLOAT, VK_IMAGE_LAYOUT_GENERAL,
                                                   VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_SAMPLED_BIT);

        for (uint32_t i = 0; i < kFramesInFlight; ++i)
        {
            reconColor_[i] = std::make_unique<Image2d>(fullRes, VK_FORMAT_R16G16B16A16_SFLOAT, VK_IMAGE_LAYOUT_GENERAL,
                                                       VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_SAMPLED_BIT);
            reconDepth_[i] = std::make_unique<Image2d>(fullRes, VK_FORMAT_R32_SFLOAT, VK_IMAGE_LAYOUT_GENERAL,
                                                       VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_SAMPLED_BIT);
            reconFog_[i]   = std::make_unique<Image2d>(fullRes, VK_FORMAT_R16G16B16A16_SFLOAT, VK_IMAGE_LAYOUT_GENERAL,
                                                       VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_SAMPLED_BIT);
        }
        ...
        // Transition all images to SHADER_READ_ONLY_OPTIMAL before BindDescriptors()
        {
            ...
            {
                Image::InsertImageMemoryBarrier(cmd, img->GetImage(), 0, VK_ACCESS_SHADER_READ_BIT,
    ...
    void CloudPipelinePass::BindDescriptors()
    {
        // --- Raymarch set ---
        auto raymarchWrites =
                DescriptorSetWriteBuilder(*raymarchSet_)
                        ...
                        .Image(10, AtmoLUTs::Get().GetTransmittanceLUT()->GetTexture()->GetView(),
                               VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL)
                        .Image(11, AtmoLUTs::Get().GetAerialPerspectiveLUT()->GetAerialPerspectiveRange()->GetView(),
                               VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL)
                        .Image(14, cloudDepthRT_->GetView(), VK_IMAGE_LAYOUT_GENERAL, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE)
                        .CombinedImageSampler(20, AtmoLUTs::Get().GetSkyViewLUT()->GetTexture()->GetView(),
                        ...
                        .CombinedImageSampler(27, AtmoLUTs::Get().GetMultiScatterLUT()->GetTexture()->GetView(),
                                              SharedSamplers::GetLinearClampSampler(),
                                              VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL)
        ...
        for (uint32_t i = 0; i < kFramesInFlight; ++i)
        {
            ...
            auto reconstructWrites =
                    DescriptorSetWriteBuilder(*reconstructSet_[i])
                            ...
                            .Image(12, reconColor_[i]->GetView(), VK_IMAGE_LAYOUT_GENERAL,
                                   VK_DESCRIPTOR_TYPE_STORAGE_IMAGE)
                            .Image(16, reconDepth_[i]->GetView(), VK_IMAGE_LAYOUT_GENERAL,
                                   VK_DESCRIPTOR_TYPE_STORAGE_IMAGE)
                            .Image(24, reconFog_[i]->GetView(), VK_IMAGE_LAYOUT_GENERAL,
                                   VK_DESCRIPTOR_TYPE_STORAGE_IMAGE)
            ...
            auto compositeWrites =
                    DescriptorSetWriteBuilder(*compositeSet_[i])
                            .Image(10, AtmoLUTs::Get().GetTransmittanceLUT()->GetTexture()->GetView(),
                                   VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL)
                            .Buffer(21, cloudUBO_->GetBuffer())
                            .CombinedImageSampler(27, AtmoLUTs::Get().GetMultiScatterLUT()->GetTexture()->GetView(),
    ...
    void CloudPipelinePass::SetFrameData(const Mat4 &invProj, const Mat4 &invView, const Vec3 &cameraPos,
    ...
    void CloudPipelinePass::PreRender(const CommandBuffer &cmd)
    {
        ...
        // ===================================================================
        {
            ...
            {
                Image::InsertImageMemoryBarrier(cmd, const_cast<Image2d *>(colorImg)->GetImage(), 0,
            ...
            } else
            {
                Image::InsertImageMemoryBarrier(
                        cmd, const_cast<Image2d *>(colorImg)->GetImage(), VK_ACCESS_SHADER_READ_BIT,
                        VK_ACCESS_SHADER_WRITE_BIT | VK_ACCESS_SHADER_READ_BIT,
        ...
        // ===================================================================
        {
            ...
            for (int i = 0; i < 3; ++i)
            {
                ...
                Image::InsertImageMemoryBarrier(
                        cmd, quarterImages[i]->GetImage(), VK_ACCESS_SHADER_READ_BIT, VK_ACCESS_SHADER_WRITE_BIT,
                        currentLayout, VK_IMAGE_LAYOUT_GENERAL, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
                        VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, VK_IMAGE_ASPECT_COLOR_BIT, 1, 0, 1, 0);
            ...
            {
                Image::InsertImageMemoryBarrier(
                        cmd, quarterImages[i]->GetImage(), VK_ACCESS_SHADER_WRITE_BIT, VK_ACCESS_SHADER_READ_BIT,
                        VK_IMAGE_LAYOUT_GENERAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
        ...
        // ===================================================================
        {
            ...
            for (int i = 0; i < 3; ++i)
            {
                ...
                Image::InsertImageMemoryBarrier(
                        cmd, reconTargets[i]->GetImage(), VK_ACCESS_SHADER_READ_BIT, VK_ACCESS_SHADER_WRITE_BIT,
                        currentLayout, VK_IMAGE_LAYOUT_GENERAL, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
                        VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, VK_IMAGE_ASPECT_COLOR_BIT, 1, 0, 1, 0);
        ...
        // ===================================================================
        {
            ...
            addImageWrite(3, cloudRenderRT_->GetView(), VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
                          VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE);
            addImageWrite(15, cloudDepthRT_->GetView(), VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
                          VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE);
            addImageWrite(23, cloudFogRT_->GetView(), VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
                          VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE);
            ...
            if (hasHistory)
            {
                ...
                addImageWrite(18, reconColor_[hist]->GetView(), VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
                              VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE);
                addImageWrite(19, reconDepth_[hist]->GetView(), VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
                              VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE);
                addImageWrite(26, reconFog_[hist]->GetView(), VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
                              VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE);
            } else
            {
                ...
                addImageWrite(18, dummyTexture_->GetView(), VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
                              VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE);
                addImageWrite(19, dummyTexture_->GetView(), VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
                              VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE);
                addImageWrite(26, dummyTexture_->GetView(), VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
                              VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE);
            }
        ...
        Image::InsertImageMemoryBarrier(cmd, const_cast<Image2d *>(colorImg)->GetImage(), VK_ACCESS_SHADER_WRITE_BIT,
                                        ...
                                        VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, VK_IMAGE_ASPECT_COLOR_BIT, 1, 0, 1, 0);
