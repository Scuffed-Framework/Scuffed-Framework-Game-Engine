...
namespace SF::Engine
{
    ...
    Image::Image(VkFilter filter, VkSamplerAddressMode addressMode, VkSampleCountFlagBits samples, VkImageLayout layout,
                 VkImageUsageFlags usage, VkFormat format, uint32_t mipLevels, uint32_t arrayLayers,
                 const UVec3 &extent) :
        extent(extent), samples(samples), usage(usage), format(format), mipLevels(mipLevels), arrayLayers(arrayLayers),
        filter(filter), addressMode(addressMode), layout(layout)
    {
    }
    ...
    VkDescriptorSetLayoutBinding Image::GetDescriptorSetLayout(uint32_t binding, VkDescriptorType descriptorType,
    ...
    {
        return static_cast<uint32_t>(std::floor(std::log2(std::max(extent.x, std::max(extent.y, extent.z)))) + 1);
    }

    VkFormat Image::FindSupportedFormat(const std::vector<VkFormat> &candidates, VkImageTiling tiling,
                            ...
                            VkImageType type)
    {
        ...
        // properties contains VkMemoryPropertyFlags (e.g. VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT).
        // Set both usage hint and required flags so VMA picks the right heap.
        allocCreateInfo.usage         = VMA_MEMORY_USAGE_AUTO;
        allocCreateInfo.requiredFlags = properties;

        VmaAllocationInfo allocInfo = {};
        VkResult result =
                vmaCreateImage(*allocator, &imageCreateInfo, &allocCreateInfo, &image, &allocation, &allocInfo);
    ...
    void Image::CreateImageView(const VkImage &image, VkImageView &imageView, VkImageViewType type, VkFormat format,
    ...
    void Image::CreateMipmaps(const VkImage &image, const UVec3 &extent, VkFormat format, VkImageLayout dstImageLayout,
    ...
    void Image::CopyBufferToImage(const VkBuffer &buffer, const VkImage &image, const UVec3 &extent,
                                  uint32_t layerCount, uint32_t baseArrayLayer)
    {
        CommandBuffer commandBuffer(true); // must begin before recording
    ...
    bool Image::CopyImage(const VkImage &srcImage, VkImage &dstImage, VmaAllocation &alloc, VkFormat srcFormat,
                          const UVec3 &extent, VkImageLayout srcImageLayout, uint32_t mipLevel, uint32_t arrayLayer)
    {
        auto physicalDevice = RenderSystem::Get()->GetPhysicalDevice();
        ...
        // optimal format).
        vkGetPhysicalDeviceFormatProperties(&*physicalDevice->GetPhysicalDevice(), surface->GetFormat().format,
                                            &formatProperties);

        if (!(formatProperties.optimalTilingFeatures & VK_FORMAT_FEATURE_BLIT_SRC_BIT))
        {
            Log::Warning("Device does not support blitting from optimal tiled images, using copy instead of "
                         "blit!\n");
            supportsBlit = false;
        }
        ...
        {
            Log::Warning("Device does not support blitting to linear tiled images, using copy instead of "
                         "blit!\n");
            supportsBlit = false;
        }
        ...
        InsertImageMemoryBarrier(commandBuffer, dstImage, 0, VK_ACCESS_TRANSFER_WRITE_BIT, VK_IMAGE_LAYOUT_UNDEFINED,
                                 ...
                                 VK_PIPELINE_STAGE_TRANSFER_BIT, VK_IMAGE_ASPECT_COLOR_BIT, 1, 0, 1, 0);

        // Transition image from previous usage to transfer source layout
        InsertImageMemoryBarrier(commandBuffer, srcImage, VK_ACCESS_MEMORY_READ_BIT, VK_ACCESS_TRANSFER_READ_BIT,
                                 srcImageLayout, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, VK_PIPELINE_STAGE_TRANSFER_BIT,
                                 VK_PIPELINE_STAGE_TRANSFER_BIT, VK_IMAGE_ASPECT_COLOR_BIT, 1, mipLevel, 1, arrayLayer);
        ...
        if (supportsBlit)
        {
            ...
            VkOffset3D blitSize = {static_cast<int32_t>(extent.x), static_cast<int32_t>(extent.y),
        ...
        InsertImageMemoryBarrier(commandBuffer, dstImage, VK_ACCESS_TRANSFER_WRITE_BIT, VK_ACCESS_MEMORY_READ_BIT,
                                 VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_GENERAL,
                                 VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT,
                                 VK_IMAGE_ASPECT_COLOR_BIT, 1, 0, 1, 0);

        // Transition back the image after the blit is done.
        InsertImageMemoryBarrier(commandBuffer, srcImage, VK_ACCESS_TRANSFER_READ_BIT, VK_ACCESS_MEMORY_READ_BIT,
                                 VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, srcImageLayout, VK_PIPELINE_STAGE_TRANSFER_BIT,
                                 VK_PIPELINE_STAGE_TRANSFER_BIT, VK_IMAGE_ASPECT_COLOR_BIT, 1, mipLevel, 1, arrayLayer);
