...
namespace SF::Engine
{
    ...
    static std::unique_ptr<Image2d> Make1x1(uint8_t r, uint8_t g, uint8_t b, uint8_t a)
    {
        ...
        return std::make_unique<Image2d>(
                std::move(bmp), VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
                VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT, VK_FILTER_NEAREST,
                VK_SAMPLER_ADDRESS_MODE_REPEAT, VK_SAMPLE_COUNT_1_BIT, false, false);
    }
    ...
    void LitMeshPipelinePass::Submit(std::shared_ptr<Mesh> mesh, const MeshMaterial &material, const Mat4 &transform)
    {
    ...
    void LitMeshPipelinePass::Render(const CommandBuffer &commandBuffer)
    {
        ...
        for (auto &dc: drawList_)
        {
            ...
            vkCmdPushConstants(commandBuffer, pipeline_->GetPipelineLayout(),
                               VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(LitPushConstants),
                               &pc);
