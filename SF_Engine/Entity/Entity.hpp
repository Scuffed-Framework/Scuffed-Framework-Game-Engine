...
namespace SF::Engine
{
    ...
    public:
        ...
        explicit Entity(const std::string& entityName, Entity* parent = nullptr)
            : name(entityName), parent(parent) {}

        // Non-copyable (owns unique_ptrs), movable if you need it.
        ...
        /**
         * @brief Constructs a brand-new entity owned by this one.
         */
        template<typename... Args>
        Entity* AddChild(Args&&... args) {
            auto child = std::make_unique<Entity>(std::forward<Args>(args)...);
            child->parent = this;
        ...
        std::unique_ptr<Entity> ReleaseChild(Entity* child) {
            auto it = std::find_if(children.begin(), children.end(),
                [child](const std::unique_ptr<Entity>& c) { return c.get() == child; });
        ...
        bool IsSelfOrDescendant(const Entity* candidate) const {
            ...
            for (auto& child : children) {
                if (child->IsSelfOrDescendant(candidate)) return true;
            }
            return false;
        }
    ...
    public:
        ...
        /**
         * @brief Creates a new root entity owned directly by the registry.
         */
        ...
        {
            if (!parent) return CreateEntity(name);
        ...
        {
            if (!entity || entity == newParent) return;
        ...
        {
            if (entity) entity->MarkForRemoval();
        }
