...
namespace SF::Engine
{
    ...
    WriteDescriptorSetInformation Image::GetWriteDescriptor(
    ...
    VkDescriptorSetLayoutBinding Image::GetDescriptorSetLayout(uint32_t binding,
    ...
    std::unique_ptr<Bitmap> Image::GetBitmap(uint32_t mipLevel, uint32_t arrayLayer) const
    {
        ...
        CopyImage(image, dstImage, dstAllocation, format, {size.x, size.y, 1}, layout, mipLevel,
                  arrayLayer);
        ...
        vkGetImageSubresourceLayout(*logicalDevice, dstImage, &dstImageSubresource,
                                    &dstSubresourceLayout);

        auto bitmap =
            std::make_unique<Bitmap>(std::make_unique<uint8_t[]>(dstSubresourceLayout.size), size);

        void *data;
        vkMapMemory(*logicalDevice, dstImageMemory, dstSubresourceLayout.offset,
                    dstSubresourceLayout.size, 0, &data);
        std::memcpy(bitmap->GetData().get(), data,
                    static_cast<std::size_t>(dstSubresourceLayout.size));
    ...
    bool Image::HasDepth(VkFormat format)
    {
        ...
        return std::find(DEPTH_FORMATS.begin(), DEPTH_FORMATS.end(), format) !=
               std::end(DEPTH_FORMATS);
    }
                            ...
                            uint32_t mipLevels, uint32_t arrayLayers, VkImageType type)
    {
        ...
        VkResult result = vmaCreateImage(*allocator, &imageCreateInfo, &allocCreateInfo, &image,
    ...
    void Image::CreateImageView(const VkImage &image, VkImageView &imageView, VkImageViewType type,
                                VkFormat format, VkImageAspectFlags imageAspect, uint32_t mipLevels,
                          ...
                          VkImageLayout srcImageLayout, uint32_t mipLevel, uint32_t arrayLayer)
    {
        ...
        // optimal format).
        vkGetPhysicalDeviceFormatProperties(*physicalDevice, surface->GetFormat().format,
                                            &formatProperties);

        if (!(formatProperties.optimalTilingFeatures & VK_FORMAT_FEATURE_BLIT_SRC_BIT))
        {
            Log::Warning(
                "Device does not support blitting from optimal tiled images, using copy instead of "
                "blit!\n");
            supportsBlit = false;
        }
        ...
        {
            Log::Warning(
                "Device does not support blitting to linear tiled images, using copy instead of "
                "blit!\n");
            supportsBlit = false;
        }

        CreateImage(dstImage, alloc, extent, VK_FORMAT_R8G8B8A8_UNORM, VK_SAMPLE_COUNT_1_BIT,
        ...
        InsertImageMemoryBarrier(commandBuffer, dstImage, 0, VK_ACCESS_TRANSFER_WRITE_BIT,
        ...
        InsertImageMemoryBarrier(
            commandBuffer, srcImage, VK_ACCESS_MEMORY_READ_BIT, VK_ACCESS_TRANSFER_READ_BIT,
        ...
        InsertImageMemoryBarrier(commandBuffer, dstImage, VK_ACCESS_TRANSFER_WRITE_BIT,
                                 VK_ACCESS_MEMORY_READ_BIT, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                                 VK_IMAGE_LAYOUT_GENERAL, VK_PIPELINE_STAGE_TRANSFER_BIT,
                                 VK_PIPELINE_STAGE_TRANSFER_BIT, VK_IMAGE_ASPECT_COLOR_BIT, 1, 0, 1,
                                 0);

        // Transition back the image after the blit is done.
        InsertImageMemoryBarrier(
            commandBuffer, srcImage, VK_ACCESS_TRANSFER_READ_BIT, VK_ACCESS_MEMORY_READ_BIT,
