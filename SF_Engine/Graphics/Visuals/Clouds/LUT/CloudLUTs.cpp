...
namespace SF::Engine
{
    ...
    {
        pipeline_ = std::make_unique<ComputePipeline>("Shaders/Clouds/BlueNoiseLUT.shader");
    ...
    // =========================================================================
    // WorleyNoiseLUT3D  (3-D R8_UNORM)
    // =========================================================================
    WorleyNoiseLUT3D::WorleyNoiseLUT3D(uint32_t size)
        : size_(size)
    {
    ...
    void WorleyNoiseLUT3D::createPipeline()
    {
        pipeline_ = std::make_unique<ComputePipeline>("Shaders/Clouds/WorleyNoiseLUT.shader");
        descSet_  = std::make_unique<DescriptorSet>(*pipeline_);
        bindStorageImage(*descSet_, texture_.get());
    }

    void WorleyNoiseLUT3D::Bake(const CommandBuffer &cmd)
    {
        if (baked_)
        ...
        texture_->SetLayout(VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
        baked_ = true;
        Log::Info("WorleyNoiseLUT3D::Bake complete ({}^3)", size_);
    }

    // =========================================================================
    // CurlNoiseLUT3D  (3-D R8G8B8A8_UNORM)
    // =========================================================================
    CurlNoiseLUT3D::CurlNoiseLUT3D(uint32_t size)
        : size_(size)
    {
        texture_ = std::make_unique<Image3d>(
            VkExtent3D{size_, size_, size_},
    ...
    void CurlNoiseLUT3D::createPipeline()
    {
        pipeline_ = std::make_unique<ComputePipeline>("Shaders/Clouds/CurlNoiseLUT.shader");
        descSet_  = std::make_unique<DescriptorSet>(*pipeline_);
        bindStorageImage(*descSet_, texture_.get());
    }

    void CurlNoiseLUT3D::Bake(const CommandBuffer &cmd)
    {
        if (baked_)
            transitionToGeneral3d(cmd, texture_.get());
        ...
        texture_->SetLayout(VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
        baked_ = true;
        Log::Info("CurlNoiseLUT3D::Bake complete ({}^3)", size_);
    }

    // =========================================================================
    // CoverageLUT  (2-D R8_UNORM)
    // =========================================================================
    CoverageLUT::CoverageLUT(uint32_t width, uint32_t height)
    ...
    void CoverageLUT::createPipeline()
    {
        pipeline_ = std::make_unique<ComputePipeline>("Shaders/Clouds/CoverageLUT.shader");
        descSet_  = std::make_unique<DescriptorSet>(*pipeline_);
        bindStorageImage(*descSet_, texture_.get());
    }

    void CoverageLUT::Bake(const CommandBuffer &cmd)
    {
        if (baked_)
            transitionToGeneral(cmd, texture_.get());
        ...
        texture_->SetLayout(VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
        baked_ = true;
        Log::Info("CoverageLUT::Bake complete ({}x{})", width_, height_);
    }

    // =========================================================================
    // ShadowLUT  (2-D R16_SFLOAT)
    // =========================================================================
    ShadowLUT::ShadowLUT(uint32_t width, uint32_t height)
        : width_(width), height_(height)
    {
        texture_ = std::make_unique<Image2d>(
            Vector2Uint{width_, height_},
    ...
    void ShadowLUT::createPipeline()
    {
        pipeline_ = std::make_unique<ComputePipeline>("Shaders/Clouds/ShadowLUT.shader");
        descSet_  = std::make_unique<DescriptorSet>(*pipeline_);
        bindStorageImage(*descSet_, texture_.get());
    }

    void ShadowLUT::Bake(const CommandBuffer &cmd)
    {
        if (baked_)
            transitionToGeneral(cmd, texture_.get());
        ...
        texture_->SetLayout(VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
        baked_ = true;
        Log::Info("ShadowLUT::Bake complete ({}x{})", width_, height_);
    }
