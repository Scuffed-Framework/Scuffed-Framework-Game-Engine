...
//  [!] Renderer: User texture binding. Use a VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE 'VkDescriptorSet' as texture identifier.
//  Call ImGui_ImplVulkan_AddTexture() to register one. Read the FAQ about ImTextureID/ImTextureRef +
//  https://github.com/ocornut/imgui/pull/914 for discussions. [X] Renderer: Large meshes support (64k+ vertices) even
//  with 16-bit indices (ImGuiBackendFlags_RendererHasVtxOffset). [X] Renderer: Texture updates support for dynamic font
//  atlas (ImGuiBackendFlags_RendererHasTextures). [X] Renderer: Expose selected render state for draw callbacks to use.
...
//  2026-07-15: [Docking] Vulkan: fixed use-after-free when using multi-viewport with dynamic rendering path: deep-copy
//  SurfaceFormat.format into the persistent buffer instead of storing a pointer to the viewport's wd->SurfaceFormat
//  which dangles after the viewport is destroyed. (#9390, #9468) 2026-04-23: Added support for standard draw callbacks
//  (in platform_io): DrawCallback_ResetRenderState, DrawCallback_SetSamplerLinear, DrawCallback_SetSamplerNearest.
//  (#9378) 2026-04-22: *BREAKING CHANGE* redesigned to use separate ImageView + Sampler instead of Combined Image
//  Sampler. This change allows us to facilitate changing samplers, in line with other backends.
...
//                - Before: need at least IMGUI_IMPL_VULKAN_MINIMUM_IMAGE_SAMPLER_POOL_SIZE descriptors of type
//                VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER.
//                - After:  need at least IMGUI_IMPL_VULKAN_MINIMUM_SAMPLED_IMAGE_POOL_SIZE descriptors of type
//                VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE.
//                                      + IMGUI_IMPL_VULKAN_MINIMUM_SAMPLER_POOL_SIZE       descriptors of type
//                                      VK_DESCRIPTOR_TYPE_SAMPLER.
//  2026-03-11: Vulkan: Added ImGui_ImplVulkan_PipelineInfo::ExtraDynamicStates[] to allow specifying extra dynamic
//  states to add when creating the VkPipeline. (#9211) 2025-09-26: [Helpers] *BREAKING CHANGE*: Vulkan: Helper
//  ImGui_ImplVulkanH_DestroyWindow() does not call vkDestroySurfaceKHR(): as surface is created by caller of
//  ImGui_ImplVulkanH_CreateOrResizeWindow(), it is more consistent that we don't destroy it. (#9163) 2026-01-05:
//  [Helpers] *BREAKING CHANGE*: Vulkan: Helper for creating render pass uses ImGui_ImplVulkanH_Window::AttachmentDesc
//  to create render pass. Removed ClearEnabled. (#9152) 2025-11-24: [Helpers] Vulkan: Helper for creating a swap-chain
//  (used by examples and multi-viewports) selects VkSwapchainCreateInfoKHR's compositeAlpha based on
//  cap.supportedCompositeAlpha. (#8784) 2025-11-13: [Docking] Handle viewport surface creation failure without
//  crashing. (#9068) 2025-10-15: Vulkan: Added IMGUI_IMPL_VULKAN_VOLK_FILENAME to configure path to volk.h header.
//  (#9008) 2025-09-26: *BREAKING CHANGE*: moved some fields in ImGui_ImplVulkan_InitInfo: init_info.RenderPass -->
...
//  affect many users. 2025-09-26: [Helpers] *BREAKING CHANGE*: Helper ImGui_ImplVulkanH_CreateOrResizeWindow() added a
...
//  ImGui_ImplVulkan_InitInfo::CustomShaderVertCreateInfo/CustomShaderFragCreateInfo. (#8585) 2025-09-22: [Docking]
//  Added ImGui_ImplVulkanH_GetWindowDataFromViewport() accessor/helper. (#8946, #8940) 2025-09-18: Call
...
//  available in descriptor pool. (#6642) 2025-01-06: [Helpers] Vulkan: Added more ImGui_ImplVulkanH_XXXX helper
//  functions to simplify our examples. 2024-12-11: [Helpers] Vulkan: Fixed setting
...
//  2024-01-19: [Helpers] Vulkan: Fixed vkAcquireNextImageKHR() validation errors in VulkanSDK 1.3.275 by allocating one
//  extra semaphore than in-flight frames. (#7236) 2024-01-11: Vulkan: Fixed vkMapMemory() calls unnecessarily using
//  full buffer size (#3957). Fixed MinAllocationSize handing (#7189). 2024-01-03: Vulkan: Added MinAllocationSize field
//  in ImGui_ImplVulkan_InitInfo to workaround zealous "best practice" validation layer. (#7189, #4238) 2024-01-03:
...
//  [Helpers] Vulkan: Added VkInstance argument to ImGui_ImplVulkanH_CreateWindow() optional helper. 2019-04-04: Vulkan:
//  Avoid passing negative coordinates to vkCmdSetScissor, which debug validation layers do not like. 2019-04-01:
//  Vulkan: Support for 32-bit index buffer (#define ImDrawIdx unsigned int). 2019-02-16: Vulkan: Viewport and clipping
//  rectangles correctly using draw_data->FramebufferScale to allow retina display. 2018-11-30: Misc: Setting up
//  io.BackendRendererName so it can be displayed in the About Window. 2018-08-25: [Helpers] Vulkan: Fixed mishandled
...
//  [Helpers] Vulkan: Various refactor, created a couple of ImGui_ImplVulkanH_XXX helper that the example can use and
//  that viewport support will use. 2018-03-01: Vulkan: Renamed ImGui_ImplVulkan_Init_Info to ImGui_ImplVulkan_InitInfo
//  and fields to match more closely Vulkan terminology. 2018-02-16: Misc: Obsoleted the io.RenderDrawListsFn callback,
...
static uint32_t __glsl_shader_vert_spv[] = {
        ...
        0x0000000f, 0x0000001e, 0x00000002, 0x00040047, 0x00000015, 0x0000001e, 0x00000001, 0x00030047, 0x00000019,
        0x00000002, 0x00050048, 0x00000019, 0x00000000, 0x0000000b, 0x00000000, 0x00040047, 0x0000001c, 0x0000001e,
        0x00000000, 0x00030047, 0x0000001e, 0x00000002, 0x00050048, 0x0000001e, 0x00000000, 0x00000023, 0x00000000,
        0x00050048, 0x0000001e, 0x00000001, 0x00000023, 0x00000008, 0x00020013, 0x00000002, 0x00030021, 0x00000003,
...
static uint32_t __glsl_shader_frag_spv[] = {
        0x07230203, 0x00010000, 0x0008000b, 0x00000023, 0x00000000, 0x00020011, 0x00000001, 0x0006000b, 0x00000001,
        ...
        0x00040006, 0x0000000b, 0x00000001, 0x00005655, 0x00030005, 0x0000000d, 0x00006e49, 0x00050005, 0x00000015,
        0x7865545f, 0x65727574, 0x00000000, 0x00050005, 0x00000019, 0x6d61535f, 0x72656c70, 0x00000000, 0x00040047,
        0x00000009, 0x0000001e, 0x00000000, 0x00040047, 0x0000000d, 0x0000001e, 0x00000000, 0x00040047, 0x00000015,
        0x00000021, 0x00000000, 0x00040047, 0x00000015, 0x00000022, 0x00000000, 0x00040047, 0x00000019, 0x00000021,
        0x00000000, 0x00040047, 0x00000019, 0x00000022, 0x00000001, 0x00020013, 0x00000002, 0x00030021, 0x00000003,
        0x00000002, 0x00030016, 0x00000006, 0x00000020, 0x00040017, 0x00000007, 0x00000006, 0x00000004, 0x00040020,
        0x00000008, 0x00000003, 0x00000007, 0x0004003b, 0x00000008, 0x00000009, 0x00000003, 0x00040017, 0x0000000a,
        0x00000006, 0x00000002, 0x0004001e, 0x0000000b, 0x00000007, 0x0000000a, 0x00040020, 0x0000000c, 0x00000001,
        0x0000000b, 0x0004003b, 0x0000000c, 0x0000000d, 0x00000001, 0x00040015, 0x0000000e, 0x00000020, 0x00000001,
        0x0004002b, 0x0000000e, 0x0000000f, 0x00000000, 0x00040020, 0x00000010, 0x00000001, 0x00000007, 0x00090019,
        0x00000013, 0x00000006, 0x00000001, 0x00000000, 0x00000000, 0x00000000, 0x00000001, 0x00000000, 0x00040020,
        0x00000014, 0x00000000, 0x00000013, 0x0004003b, 0x00000014, 0x00000015, 0x00000000, 0x0002001a, 0x00000017,
        0x00040020, 0x00000018, 0x00000000, 0x00000017, 0x0004003b, 0x00000018, 0x00000019, 0x00000000, 0x0003001b,
        0x0000001b, 0x00000013, 0x0004002b, 0x0000000e, 0x0000001d, 0x00000001, 0x00040020, 0x0000001e, 0x00000001,
        0x0000000a, 0x00050036, 0x00000002, 0x00000004, 0x00000000, 0x00000003, 0x000200f8, 0x00000005, 0x00050041,
        0x00000010, 0x00000011, 0x0000000d, 0x0000000f, 0x0004003d, 0x00000007, 0x00000012, 0x00000011, 0x0004003d,
        0x00000013, 0x00000016, 0x00000015, 0x0004003d, 0x00000017, 0x0000001a, 0x00000019, 0x00050056, 0x0000001b,
        0x0000001c, 0x00000016, 0x0000001a, 0x00050041, 0x0000001e, 0x0000001f, 0x0000000d, 0x0000001d, 0x0004003d,
        0x0000000a, 0x00000020, 0x0000001f, 0x00050057, 0x00000007, 0x00000021, 0x0000001c, 0x00000020, 0x00050085,
                                              ...
                                              int fb_width, int fb_height)
{
    ...
    // Setup sampler
    vkCmdBindDescriptorSets(bd->RenderState->CommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, bd->PipelineLayout, 1, 1,
                            &bd->SamplerLinearDS, 0, nullptr);
}

// Draw callbacks
static void ImGui_ImplVulkan_DrawCallback_ResetRenderState(const ImDrawList *, const ImDrawCmd *) {
...
static void ImGui_ImplVulkan_DrawCallback_SetSamplerLinear(const ImDrawList *, const ImDrawCmd *)
{
    ...
    vkCmdBindDescriptorSets(bd->RenderState->CommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                            bd->RenderState->PipelineLayout, 1, 1, &bd->SamplerLinearDS, 0, nullptr);
}
static void ImGui_ImplVulkan_DrawCallback_SetSamplerNearest(const ImDrawList *, const ImDrawCmd *)
{
    ImGui_ImplVulkan_Data *bd = ImGui_ImplVulkan_GetBackendData();
    vkCmdBindDescriptorSets(bd->RenderState->CommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                            bd->RenderState->PipelineLayout, 1, 1, &bd->SamplerNearestDS, 0, nullptr);
}

// If you want to use your own sampler, you can create your own callback, access ImGui_ImplVulkan_RenderState for
// command-buffer and pipeline layout, e.g.
/*
...
(ImGui_ImplVulkan_RenderState*)ImGui::GetPlatformIO().Renderer_RenderState;
    vkCmdBindDescriptorSets(render_state->CommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, render_state->PipelineLayout,
1, 1, (VkDescriptorSet*)cmd->UserCallbackData, 0, nullptr);
}
*/
...
void ImGui_ImplVulkan_UpdateTexture(ImTextureData *tex)
{
    ...
    if (tex->Status == ImTextureStatus_WantCreate)
    {
        ...
        backend_tex->DescriptorSet =
                ImGui_ImplVulkan_AddTexture(backend_tex->ImageView, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
                                     ...
                                     const VkAllocationCallbacks *allocator)
{
    ...
    // vkDestroySurfaceKHR(instance, wd->Surface, allocator); // v1.92.6 (~2026-01-16): because wd->Surface is user
    // provided we don't attempt to destroy it ourself.
}
...
static void ImGui_ImplVulkan_CreateWindow(ImGuiViewport *viewport)
{
    ...
    wd->SurfaceFormat =
            ImGui_ImplVulkanH_SelectSurfaceFormat(v->PhysicalDevice, wd->Surface, requestSurfaceImageFormats.Data,
                                                  requestSurfaceImageFormats.Size, requestSurfaceColorSpace);
    ...
    wd->AttachmentDesc.loadOp = (viewport->Flags & ImGuiViewportFlags_NoRendererClear) ? VK_ATTACHMENT_LOAD_OP_DONT_CARE
