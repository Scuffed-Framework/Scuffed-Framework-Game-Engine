...
namespace libmatroska
{
    ...
    */
    ...
    KaxSeek* KaxSeekHead::FindNextOf(const KaxSeek& aPrev) const
    {
        ...
        auto it2 = std::find_if(it + 1, this->end(),
                                [&](auto e)
                                {
                                    return (EbmlId(*(*it)) == EBML_ID(KaxSeek)) &&
                                           static_cast<KaxSeek*>(e)->IsEbmlId(aPrev);
                                });
        if (it2 == this->end()) return nullptr;
