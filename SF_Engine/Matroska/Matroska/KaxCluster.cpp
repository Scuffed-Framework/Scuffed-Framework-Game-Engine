...
namespace libmatroska
{
    ...
    KaxCluster::KaxCluster() : EbmlMaster(KaxCluster::ClassInfos) {}

    KaxCluster::KaxCluster(const KaxCluster& ElementToClone) : EbmlMaster(ElementToClone)
    {
    ...
    bool KaxCluster::AddFrameInternal(const KaxTrackEntry& track, std::uint64_t timestamp,
                                      DataBuffer& buffer, KaxBlockGroup*& MyNewBlock,
                                      const KaxBlockGroup* PastBlock,
                                      const KaxBlockGroup* ForwBlock, LacingType lacing)
    {
        ...
        if (!currentNewBlock ||
            static_cast<std::uint32_t>(track.TrackNumber()) !=
                static_cast<std::uint32_t>(currentNewBlock->TrackNumber()) ||
            PastBlock || ForwBlock)
        {
        ...
        if (PastBlock)
        {
            if (ForwBlock)
            {
                if (currentNewBlock->AddFrame(track, timestamp, buffer, *PastBlock, *ForwBlock,
                                              lacing))
                {
    ...
    bool KaxCluster::AddFrame(const KaxTrackEntry& track, std::uint64_t timestamp,
                              DataBuffer& buffer, KaxBlockGroup*& MyNewBlock, LacingType lacing)
    {
    ...
    bool KaxCluster::AddFrame(const KaxTrackEntry& track, std::uint64_t timestamp,
                              DataBuffer& buffer, KaxBlockGroup*& MyNewBlock,
                              const KaxBlockGroup& PastBlock, LacingType lacing)
    {
        assert(Blobs.empty());  // mutually exclusive for the moment
    ...
    {
        assert(Blobs.empty());  // mutually exclusive for the moment
        return AddFrameInternal(track, timestamp, buffer, MyNewBlock, &PastBlock, &ForwBlock,
                                lacing);
    }
    ...
    */
    filepos_t KaxCluster::Render(IOCallback& output, KaxCues& CueToUpdate,
                                 const ShouldWrite& writeFilter)
    {
        ...
        else
        {
            ...
            // For all Blocks add their position on the CueEntry
            for (const auto& blob : Blobs) CueToUpdate.PositionSet(*blob);
    ...
    /*!
      \todo automatically choose valid timestamp for the Cluster based on the previous cluster
      timestamp (must be incremental)
    */
    std::uint64_t KaxCluster::GlobalTimestamp() const
    {
        ...
        if (result < PreviousTimestamp) result = PreviousTimestamp + 1;
    ...
    */
    ...
    {
        const std::int64_t TimestampDelay = (static_cast<std::int64_t>(aGlobalTimestamp) -
                                             static_cast<std::int64_t>(GlobalTimestamp())) /
                                            static_cast<std::int64_t>(GlobalTimestampScale());
    ...
    std::uint64_t KaxCluster::GetBlockGlobalTimestamp(std::int16_t LocalTimestamp)
    {
        if (!bFirstFrameInside)
        {
            ...
            MinTimestamp = MaxTimestamp = PreviousTimestamp =
                static_cast<std::uint64_t>(*static_cast<EbmlUInteger*>(ClusterTimestamp));
            bFirstFrameInside = true;
            bPreviousTimestampIsSet = true;
        }
        return static_cast<std::int64_t>(LocalTimestamp * GlobalTimestampScale()) +
               GlobalTimestamp();
    }
