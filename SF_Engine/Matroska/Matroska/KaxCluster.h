...
public:
    /*!
      ...
      \param timestamp the timestamp is expressed in nanoseconds, relative to the beggining of the
      Segment
    */
    bool AddFrame(const KaxTrackEntry& track, std::uint64_t timestamp, DataBuffer& buffer,
                  KaxBlockGroup*& MyNewBlock, LacingType lacing = LACING_AUTO);
    /*!
    ...
    */
    bool AddFrame(const KaxTrackEntry& track, std::uint64_t timestamp, DataBuffer& buffer,
                  KaxBlockGroup*& MyNewBlock, const KaxBlockGroup& PastBlock,
    ...
    */
    bool AddFrame(const KaxTrackEntry& track, std::uint64_t timestamp, DataBuffer& buffer,
                  KaxBlockGroup*& MyNewBlock, const KaxBlockGroup& PastBlock,
                  const KaxBlockGroup& ForwBlock, LacingType lacing = LACING_AUTO);

    /*!
      \brief Render the data to the stream and retrieve the position of BlockGroups for later cue
      entries
    */
    libebml::filepos_t Render(libebml::IOCallback& output, KaxCues& CueToUpdate,
    ...
    */
    ...
    /*!
      ...
      \note this is a convenience to be able to keep Clusters+Blocks in memory (for future
      reference) withouht being a memory hog
    */
    void ReleaseFrames();
    ...
    */
    ...
    void SetParent(const KaxSegment& aParentSegment)
    {
        ParentSegment = &aParentSegment;
    }
    ...
    */
    ...
    const KaxSegment* GetParentSegment() const
    {
...
protected:
    ...
    */
    bool AddFrameInternal(const KaxTrackEntry& track, std::uint64_t timestamp, DataBuffer& buffer,
                          KaxBlockGroup*& MyNewBlock, const KaxBlockGroup* PastBlock,
