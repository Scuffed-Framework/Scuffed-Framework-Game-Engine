...
namespace libmatroska
{
    ...
    */
    void KaxCuePoint::PositionSet(const KaxBlockGroup& BlockReference,
                                  std::uint64_t GlobalTimestampScale)
    {
        // fill me
        ...
        if (CodecState)
        {
            ...
            CueCodecState.SetValue(
                BlockReference.GetParentCluster()->GetParentSegment()->GetRelativePosition(
    ...
    void KaxCuePoint::PositionSet(const KaxBlockBlob& BlobReference,
                                  std::uint64_t GlobalTimestampScale)
    {
    ...
    void KaxCuePoint::PositionSet(const KaxSimpleBlock& BlockReference,
                                  std::uint64_t GlobalTimestampScale)
    {
        PositionSet(BlockReference, nullptr, GlobalTimestampScale);
    }

    void KaxCuePoint::PositionSet(const KaxInternalBlock& BlockReference,
                                  const KaxBlockGroup* BlockGroup,
                                  std::uint64_t GlobalTimestampScale)
    {
        // fill me
        auto& NewTime = GetChild<KaxCueTime>(*this);
  ...
  if (BlockReference.ReferenceCount() != 0) {
      ...
      NewRefs.AddReference(BlockReference.Reference(i).RefBlock(), GlobalTimestampScale);
        ...
        if (BlockGroup)
        {
            ...
            if (CodecState)
            {
                auto& CueCodecState = AddNewChild<KaxCueCodecState>(NewPositions);
                CueCodecState.SetValue(
                    BlockGroup->GetParentCluster()->GetParentSegment()->GetRelativePosition(
                        CodecState->GetElementPosition()));
            }
        }
    ...
    /*!
      \todo handle codec state checking
    */
    void KaxCueReference::AddReference(const KaxBlockBlob& BlockReference,
                                       std::uint64_t GlobalTimestampScale)
    {
    ...
    bool KaxCuePoint::IsSmallerThan(const EbmlElement* Cmp) const
    {
        ...
        if (TimestampA->IsSmallerThan(TimestampB)) return true;

        if (TimestampB->IsSmallerThan(TimestampA)) return false;
        ...
        if (TrackA->IsSmallerThan(TrackB)) return true;

        if (TrackB->IsSmallerThan(TrackA)) return false;
