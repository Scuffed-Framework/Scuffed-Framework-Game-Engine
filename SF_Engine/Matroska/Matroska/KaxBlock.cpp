...
#include <Matroska/EBML/EbmlEndian.h>
#include <Matroska/EBML/MemReadIOCallback.h>
#include <Matroska/EBML/SafeReadIOCallback.h>
#include "KaxBlock.h"
...
namespace libmatroska
{
    ...
    SimpleDataBuffer::SimpleDataBuffer(const SimpleDataBuffer& ToClone)
        : DataBuffer(static_cast<binary*>(malloc(ToClone.mySize * sizeof(binary))), ToClone.mySize,
                     myFreeBuffer)
    {
        assert(myBuffer);
          ...
          ParentCluster(ElementToClone.ParentCluster)  ///< \todo not exactly
    {
        ...
        for (const auto& buffer : ElementToClone.myBuffers) myBuffers.push_back(buffer->Clone());
    }
    ...
    KaxBlockGroup::KaxBlockGroup() : EbmlMaster(KaxBlockGroup::ClassInfos) {}
...
#define SignedVINT_MIN(n) (-(SignedVINT_Shift##n) - 1)
    ...
    */
    bool KaxInternalBlock::AddFrame(const KaxTrackEntry& track, std::uint64_t timestamp,
                                    DataBuffer& buffer, LacingType lacing, bool invisible)
    {
        SetValueIsSet();
        if (myBuffers.empty())
        {
        ...
        if (myBuffers.size() >= 8 || lacing == LACING_NONE) return false;

        if (lacing == LACING_XIPH)
            // decide wether a new frame can be added or not
            // a frame in a lace is not efficient when the place necessary to code it in a lace is
            // bigger than the size of a simple Block. That means more than 6 bytes (4 in struct + 2
            // for EBML) to code the size
    ...
    */
    LacingType KaxInternalBlock::GetBestLacingType() const
    {
        ...
        for (i = 1; i < static_cast<int>(myBuffers.size()) - 1; i++)
            EbmlLacingSize += SignedVINTLength(static_cast<std::int64_t>(myBuffers[i]->Size()) -
                                               static_cast<std::int64_t>(myBuffers[i - 1]->Size()));
        if (SameSize) return LACING_FIXED;
        if (XiphLacingSize < EbmlLacingSize) return LACING_XIPH;
                    ...
                    case LACING_XIPH:
                        for (i = 0; i < myBuffers.size() - 1; i++)
                        {
                            SetSize_(GetSize() + myBuffers[i]->Size() +
                                     (myBuffers[i]->Size() / 0xFF + 1));
                        }
                        break;
                    case LACING_EBML:
                        SetSize_(GetSize() + myBuffers[0]->Size() +
                                 CodedSizeLength(myBuffers[0]->Size(), 0, IsFiniteSize()));
                        for (i = 1; i < myBuffers.size() - 1; i++)
                        {
                            SetSize_(GetSize() + myBuffers[i]->Size() +
                                     SignedVINTLength(
                                         static_cast<std::int64_t>(myBuffers[i]->Size()) -
                                         static_cast<std::int64_t>(myBuffers[i - 1]->Size())));
                        }
                        break;
                    case LACING_FIXED:
    ...
    KaxBlockVirtual::KaxBlockVirtual() : EbmlBinary(KaxBlockVirtual::ClassInfos)
    {
        SetBuffer(DataBlock, sizeof(DataBlock));
        SetValueIsSet(false);
    }
    ...
    {
        if (GetBuffer() == DataBlock) SetBuffer(nullptr, 0);
    }
    ...
    /*!
      \todo more optimisation is possible (render the Block head and don't copy the buffer in
      memory, care should be taken with the allocation of Data)
      \todo the actual timestamp to write should be retrieved from the Cluster from here
    */
    filepos_t KaxInternalBlock::RenderData(IOCallback& output, bool /* bForceRender */,
                                           const ShouldWrite&)
    {
        ...
        {
            if (mLacing == LACING_NONE) mLacing = LACING_EBML;  // supposedly the best of all
            SetSize_(4 + 1);  // 1 for the lacing head (number of laced elements)
        }
        if (TrackNumber >= 0x80) SetSize_(GetSize() + 1);
        ...
        *cursor = 0;  // flags
        ...
        // invisible flag
        if (mInvisible) *cursor = 0x08;

        if (static_cast<const EbmlId&>(*this) == EBML_ID(KaxSimpleBlock))
        {
            auto* s = reinterpret_cast<const KaxSimpleBlock*>(this);
            if (s->IsKeyframe()) *cursor |= 0x80;
            if (s->IsDiscardable()) *cursor |= 0x01;
        }
        ...
        switch (mLacing)
        {
            ...
            case LACING_EBML:
                ...
                output.writeFully(&tmpValue, 1);
                {
                    ...
                    for (i = 1; i < myBuffers.size() - 1; i++)
                    {
                        _Size = static_cast<std::int64_t>(myBuffers[i]->Size()) -
                                static_cast<std::int64_t>(myBuffers[i - 1]->Size());
    ...
    */
    filepos_t KaxInternalBlock::ReadData(IOCallback& input, ScopeMode ReadFully)
    {
        ...
        try
        {
            if (ReadFully == SCOPE_ALL_DATA)
            {
                ...
                if (Result != GetSize()) throw SafeReadIOCallback::EndOfStreamX(GetSize() - Result);
                ...
                if (mLacing == LACING_NONE)
                {
                    ...
                    auto soloFrame =
                        new DataBuffer(BufferStart + Mem.GetPosition(), GetSize() - BlockHeadSize);
                ...
                else
                {
                    ...
                    std::uint32_t LastBufferSize =
                        GetSize() - BlockHeadSize - 1;  // 1 for number of frame
                    const std::uint8_t FrameNum =
                        Mem.GetUInt8();  // number of frames in the lace - 1
                    ...
                    switch (mLacing)
                    {
                        ...
                        case LACING_EBML:
                            ...
                            FrameSize = ReadCodedSizeValue(BufferStart + Mem.GetPosition(),
                                                           SizeRead, SizeUnknown);
                            if (!FrameSize ||
                                (static_cast<std::uint32_t>(FrameSize + SizeRead) > LastBufferSize))
                            ...
                            for (Index = 1; Index < FrameNum; Index++)
                            {
                                ...
                                SizeRead = LastBufferSize;
                                FrameSize +=
                                    ReadSignedVINT(BufferStart + Mem.GetPosition(), SizeRead);
                                if (!FrameSize || (static_cast<std::uint32_t>(
                                                       FrameSize + SizeRead) > LastBufferSize))
                                    throw SafeReadIOCallback::EndOfStreamX(SizeRead);
                                SizeList[Index] = FrameSize;
                                Mem.Skip(SizeRead);
                                LastBufferSize -= FrameSize + SizeRead;
                    ...
                    {
                        auto lacedFrame =
                            new DataBuffer(BufferStart + Mem.GetPosition(), SizeList[Index]);
                ...
                for (std::size_t Index = 0; Index < NumFrames; ++Index)
                {
                    ...
                    const auto ExpectedEnd =
                        (Index + 1) < NumFrames ? myBuffers[Index + 1]->Buffer() : BufferEnd;

                    if ((FrameStart < BufferStart) || (FrameEnd > BufferEnd) ||
            ...
            else if (ReadFully == SCOPE_PARTIAL_DATA)
            {
                ...
                if (Result != 5) throw SafeReadIOCallback::EndOfStreamX(0);
                ...
                {
                    // read the number of frames in the lace
                    const std::uint32_t TotalLacedSize =
                        GetSize() - BlockHeadSize - 1;  // 1 for number of frame
                    ...
                    switch (mLacing)
                    {
                        ...
                        case LACING_EBML:
                            SizeRead = LastBufferSize;
                            cursor = _tmpBuf =
                                new binary[FrameNum * 4];  /// \warning assume the mean size will be
                                                           /// coded in less than 4 bytes
    ...
    bool KaxBlockGroup::AddFrame(const KaxTrackEntry& track, std::uint64_t timestamp,
    ...
    bool KaxBlockGroup::AddFrame(const KaxTrackEntry& track, std::uint64_t timestamp,
                                 DataBuffer& buffer, const KaxBlockGroup& PastBlock,
                                 LacingType lacing)
    {
        auto& theBlock = GetChild<KaxBlock>(*this);
        assert(ParentCluster);
    ...
    bool KaxBlockGroup::AddFrame(const KaxTrackEntry& track, std::uint64_t timestamp,
                                 DataBuffer& buffer, const KaxBlockGroup& PastBlock,
                                 const KaxBlockGroup& ForwBlock, LacingType lacing)
    {
        auto& theBlock = GetChild<KaxBlock>(*this);
        assert(ParentCluster);
        theBlock.SetParent(*ParentCluster);
    ...
    bool KaxBlockGroup::AddFrame(const KaxTrackEntry& track, std::uint64_t timestamp,
                                 DataBuffer& buffer, const KaxBlockBlob* PastBlock,
                                 const KaxBlockBlob* ForwBlock, LacingType lacing)
    {
        auto& theBlock = GetChild<KaxBlock>(*this);
        assert(ParentCluster);
        theBlock.SetParent(*ParentCluster);
    ...
    */
    ...
    bool KaxBlockGroup::GetBlockDuration(std::uint64_t& TheTimestamp) const
    {
        ...
        assert(ParentTrack);
        TheTimestamp =
            static_cast<std::uint64_t>(*myDuration) * ParentTrack->GlobalTimestampScale();
        return true;
    }
    ...
    {
        assert(Block.group);
        if (bUseSimpleBlock) return *Block.simpleblock;
        return *Block.group;
    }
    ...
    bool KaxBlockBlob::AddFrameAuto(const KaxTrackEntry& track, std::uint64_t timestamp,
                                    DataBuffer& buffer, LacingType lacing,
                                    const KaxBlockBlob* PastBlock, const KaxBlockBlob* ForwBlock)
    {
        bool bResult = false;
        if ((SimpleBlockMode == BLOCK_BLOB_ALWAYS_SIMPLE) ||
            (SimpleBlockMode == BLOCK_BLOB_SIMPLE_AUTO && !PastBlock && !ForwBlock))
        {
            ...
            else
            {
                ...
                if ((!ForwBlock ||
                     static_cast<KaxInternalBlock&>(*ForwBlock).GlobalTimestamp() <= timestamp) &&
                    (!PastBlock ||
                     static_cast<KaxInternalBlock&>(*PastBlock).GlobalTimestamp() <= timestamp))
                    Block.simpleblock->SetDiscardable(false);
                else
                    Block.simpleblock->SetDiscardable(true);
            }
        }
        else if (ReplaceSimpleByGroup())
    ...
    {
        if (ReplaceSimpleByGroup()) Block.group->SetBlockDuration(TimeLength);
    }

    bool KaxBlockBlob::ReplaceSimpleByGroup()
    {
        if (SimpleBlockMode == BLOCK_BLOB_ALWAYS_SIMPLE) return false;
        ...
        if (ParentCluster) Block.group->SetParent(*ParentCluster);
