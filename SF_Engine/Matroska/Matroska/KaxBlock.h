...
namespace libmatroska
{
    ...
    public:
        DataBuffer(libebml::binary* aBuffer, std::uint32_t aSize,
                   bool (*aFreeBuffer)(const DataBuffer& aBuffer) = nullptr,
                   bool _bInternalBuffer = false)
            : mySize(aSize), myFreeBuffer(aFreeBuffer), bInternalBuffer(_bInternalBuffer)
        {
            if (bInternalBuffer)
            {
                try
        ...
        virtual const libebml::binary* Buffer() const
        {
            assert(bValidValue);
            return myBuffer;
        }
        ...
        bool FreeBuffer(const DataBuffer& aBuffer)
        {
            ...
            {
                if (myFreeBuffer) bResult = myFreeBuffer(aBuffer);
                if (bInternalBuffer) delete[] myBuffer;
                myBuffer = nullptr;
                mySize = 0;
                bValidValue = false;
            }
    ...
    {
    public:
        SimpleDataBuffer(libebml::binary* aBuffer, std::uint32_t aSize, std::uint32_t aOffset,
                         bool (*aFreeBuffer)(const DataBuffer& aBuffer) = myFreeBuffer)
        ...
        DataBuffer* Clone() override
        {
            return new SimpleDataBuffer(*this);
        }
...
public:
    ...
    */
    bool AddFrame(const KaxTrackEntry& track, std::uint64_t timestamp, DataBuffer& buffer,
                  LacingType lacing = LACING_AUTO);
    /*!
      \brief Addition of a frame with a backward reference (P frame)
    */
    bool AddFrame(const KaxTrackEntry& track, std::uint64_t timestamp, DataBuffer& buffer,
                  const KaxBlockGroup& PastBlock, LacingType lacing = LACING_AUTO);

    /*!
      \brief Addition of a frame with a backward+forward reference (B frame)
    */
    bool AddFrame(const KaxTrackEntry& track, std::uint64_t timestamp, DataBuffer& buffer,
                  const KaxBlockGroup& PastBlock, const KaxBlockGroup& ForwBlock,
                  LacingType lacing = LACING_AUTO);
    bool AddFrame(const KaxTrackEntry& track, std::uint64_t timestamp, DataBuffer& buffer,
...
{
public:
    KaxInternalBlock(const libebml::EbmlCallbacks& classInfo) : libebml::EbmlBinary(classInfo) {}
    ...
    */
    ...
    /*!
      \note override this function to generate the Data/Size on the fly, unlike the usual binary
      elements
    */
    libebml::filepos_t UpdateSize(const ShouldWrite& writeFilter = WriteSkipDefault,
                                  bool bForceRender = false) override;
    libebml::filepos_t ReadData(libebml::IOCallback& input,
                                libebml::ScopeMode ReadFully = libebml::SCOPE_ALL_DATA) override;
    ...
    */
    ...
    unsigned int NumberFrames() const
    {
        return SizeList.size();
    }
    DataBuffer& GetBuffer(unsigned int iIndex)
    {
        return *myBuffers[iIndex];
    }

    bool AddFrame(const KaxTrackEntry& track, std::uint64_t timestamp, DataBuffer& buffer,
                  LacingType lacing = LACING_AUTO, bool invisible = false);
    ...
    */
     ...
     */
    std::int16_t GetRelativeTimestamp() const
    {
     ...
     */
    LacingType GetCurrentLacing() const
    {
        return mLacing;
    }
...
protected:
    ...
    libebml::filepos_t RenderData(libebml::IOCallback& output, bool bForceRender,
                                  const ShouldWrite& writeFilter = WriteSkipDefault) override;
};

DECLARE_xxx_BASE(KaxBlock, KaxInternalBlock) MATROSKA_CLASS_BODY(KaxBlock)
}
;

DECLARE_xxx_BASE(KaxSimpleBlock, KaxInternalBlock) private : bool bIsKeyframe{true};
bool bIsDiscardable{false};

public:
void SetKeyframe(bool b_keyframe)
{
    bIsKeyframe = b_keyframe;
}
void SetDiscardable(bool b_discard)
{
    bIsDiscardable = b_discard;
}
...
public:
    ...
    void SetParent(KaxCluster& aParentCluster);
    bool AddFrameAuto(const KaxTrackEntry& track, std::uint64_t timestamp, DataBuffer& buffer,
                      LacingType lacing = LACING_AUTO, const KaxBlockBlob* PastBlock = nullptr,
                      const KaxBlockBlob* ForwBlock = nullptr);

    bool IsSimpleBlock() const
    {
        return bUseSimpleBlock;
    }
...
void SetParent(const KaxCluster& aParentCluster)
{
