...
#include <Matroska/EBML/EbmlBinary.h>
#include <Matroska/EBML/EbmlDate.h>
#include <Matroska/EBML/EbmlFloat.h>
#include <Matroska/EBML/EbmlMaster.h>
#include <Matroska/EBML/EbmlSInteger.h>
#include <Matroska/EBML/EbmlString.h>
#include <Matroska/EBML/EbmlUInteger.h>
#include <Matroska/EBML/EbmlUnicodeString.h>
...
public:
libebml::filepos_t RenderData(libebml::IOCallback& output, bool bForceRender,
                              const ShouldWrite& writeFilter) override;
}
;
...
/**
 *The TrackType defines the type of each frame found in the Track. The value **SHOULD** be stored on
 * 1 octet.
 */
typedef enum
{
    ...
    MATROSKA_TRACK_TYPE_COMPLEX =
        0x3,  // A mix of different other TrackType. The codec needs to define how the Matroska
              // Player should interpret such data.
    MATROSKA_TRACK_TYPE_LOGO = 0x10,  // An image to be rendered over the video track(s).
    MATROSKA_TRACK_TYPE_SUBTITLE =
        0x11,  // Subtitle or closed caption data to be rendered over the video track(s).
    MATROSKA_TRACK_TYPE_BUTTONS =
        0x12,  // Interactive button(s) to be rendered over the video track(s).
    MATROSKA_TRACK_TYPE_CONTROL =
        0x20,  // Metadata used to control the player of the Matroska Player.
    MATROSKA_TRACK_TYPE_METADATA =
        0x21,  // Timed metadata that can be passed on to the Matroska Player.
} MatroskaTrackType;
...
typedef enum
{
    ...
    MATROSKA_TRACK_ENCODING_COMP_LZO1X =
        2,  // Lempel-Ziv-Oberhumer compression (LZO) **SHOULD NOT** be used.
    MATROSKA_TRACK_ENCODING_COMP_HEADERSTRIP =
        3,  // Octets in ContentCompSettings ((#contentcompsettings-element)) have been stripped
            // from each frame.
...
typedef enum
{
    ...
    MATROSKA_VIDEO_FIELDORDER_TOPFIELDFIRST =
        1,  // Top field displayed first. Top field stored first.
    MATROSKA_VIDEO_FIELDORDER_UNDETERMINED = 2,  // Unknown field order.
    MATROSKA_VIDEO_FIELDORDER_BOTTOMFIELDFIRST =
        6,  // Bottom field displayed first. Bottom field stored first.
    MATROSKA_VIDEO_FIELDORDER_TOPFIELDSWAPPED =
        9,  // Top field displayed first. Fields are interleaved in storage with the top line of the
            // top field stored first.
    MATROSKA_VIDEO_FIELDORDER_BOTTOMFIELDSWAPPED =
        14,  // Bottom field displayed first. Fields are interleaved in storage with the top line of
...
/**
 *Indicates whether the BlockAdditional element with BlockAddID of "1"     contains Alpha data as
 * defined by the Codec Mapping for the CodecID.  Undefined values (i.e., values other than 0 or 1)
 * **SHOULD NOT** be used, as the  behavior of known implementations is different.
 */
typedef enum
{
    MATROSKA_VIDEO_ALPHAMODE_NONE =
        0,  // The BlockAdditional element with BlockAddID of "1" does not exist or **SHOULD NOT**
            // be considered as containing such data.
    MATROSKA_VIDEO_ALPHAMODE_PRESENT =
        1,  // The BlockAdditional element with BlockAddID of "1" contains alpha channel data.
...
/**
 *The Matrix Coefficients of the video used to derive luma and chroma values from red, green, and
 * blue color primaries. For clarity, the value and meanings for MatrixCoefficients are adopted from
 * Table 4 of ITU-H.273.
 */
typedef enum
{
...
/**
 *The color primaries of the video. For clarity, the value and meanings for Primaries are adopted
 * from Table 2 of ITU-H.273.
 */
typedef enum
{
...
/**
 *Audio emphasis applied on audio samples. The player **MUST** apply the inverse emphasis to get the
 * proper audio samples.
 */
typedef enum
{
    MATROSKA_EMPHASIS_NO_EMPHASIS = 0,
    MATROSKA_EMPHASIS_CD_AUDIO =
        1,  // First order filter with zero point at 50 microseconds and a pole at 15 microseconds.
            // Also found on DVD Audio and MPEG audio.
    MATROSKA_EMPHASIS_RESERVED = 2,
    MATROSKA_EMPHASIS_CCIT_J_17 = 3,  // Defined in (ITU-J.17).
    MATROSKA_EMPHASIS_FM_50 =
        4,  // FM Radio in Europe. RC Filter with a time constant of 50 microseconds.
    MATROSKA_EMPHASIS_FM_75 =
        5,  // FM Radio in the USA. RC Filter with a time constant of 75 microseconds.
    MATROSKA_EMPHASIS_PHONO_RIAA = 10,  // Phono filter with time constants of t1=3180, t2=318 and
                                        // t3=75 microseconds. (NAB1964)
    MATROSKA_EMPHASIS_PHONO_IEC_N78 =
        11,  // Phono filter with time constants of t1=3180, t2=450 and t3=50 microseconds.
    MATROSKA_EMPHASIS_PHONO_TELDEC =
        12,  // Phono filter with time constants of t1=3180, t2=318 and t3=50 microseconds.
    MATROSKA_EMPHASIS_PHONO_EMI =
        13,  // Phono filter with time constants of t1=2500, t2=500 and t3=70 microseconds.
    MATROSKA_EMPHASIS_PHONO_COLUMBIA_LP =
        14,  // Phono filter with time constants of t1=1590, t2=318 and t3=100 microseconds.
    MATROSKA_EMPHASIS_PHONO_LONDON =
        15,  // Phono filter with time constants of t1=1590, t2=318 and t3=50 microseconds.
    MATROSKA_EMPHASIS_PHONO_NARTB =
        16,  // Phono filter with time constants of t1=3180, t2=318 and t3=100 microseconds.
...
/**
 *A bit field that describes which elements have been modified in this way. Values (big-endian) can
 * be OR'ed.
 */
typedef enum
{
    ...
    MATROSKA_CONTENTENCODINGSCOPE_NEXT =
        0x4,  // The next ContentEncoding (next ContentEncodingOrder; the data inside
...
/**
 *Indicates what type of content the ChapterAtom contains and might be skipped.     It can be used
 * to automatically skip content based on the type.     If a ChapterAtom is inside a ChapterAtom
 * that has a ChapterSkipType set, it     **MUST NOT** have a ChapterSkipType or have a
 * ChapterSkipType with the same value as it's parent ChapterAtom. If the ChapterAtom doesn't
 * contain a ChapterTimeEnd, the value of the ChapterSkipType is only valid until the next
 * ChapterAtom with a ChapterSkipType value or the end of the file.
 */
typedef enum
{
    MATROSKA_CHAPTERSKIPTYPE_NO_SKIPPING = 0,  // Content which should not be skipped.
    MATROSKA_CHAPTERSKIPTYPE_OPENING_CREDITS =
    ...
    MATROSKA_CHAPTERSKIPTYPE_RECAP =
        3,  // Recap of previous episodes of the content, usually found around the beginning.
    MATROSKA_CHAPTERSKIPTYPE_NEXT_PREVIEW =
        4,  // Preview of the next episode of the content, usually found around the end. It may
            // contain spoilers the user wants to avoid.
    MATROSKA_CHAPTERSKIPTYPE_PREVIEW =
        5,  // Preview of the current episode of the content, usually found around the beginning. It
...
typedef enum
{
    MATROSKA_CHAPPROCESSCODECID_MATROSKA_SCRIPT =
        0,                                     // Chapter commands using the Matroska Script codec.
...
typedef enum
{
    ...
    MATROSKA_TARGET_TYPE_SUBTRACK =
        20,  // Corresponds to parts of a track for audio, such as a movement or scene in a movie.
    ...
    MATROSKA_TARGET_TYPE_ALBUM =
        50,  // The most common grouping level of music and video (e.g., an episode for TV series).
