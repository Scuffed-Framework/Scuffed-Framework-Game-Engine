...
namespace
{
    ...
    std::int64_t ToSigned(std::uint64_t u)
    {
        if (u <= static_cast<std::uint64_t>(std::numeric_limits<std::int64_t>::max()))
            return static_cast<std::int64_t>(u);

        return static_cast<std::int64_t>(u - std::numeric_limits<std::int64_t>::min()) +
...
namespace libebml
{
    ...
    EbmlSInteger::operator std::int8_t() const
    {
        return static_cast<std::int8_t>(GetValue());
    }
    EbmlSInteger::operator std::int16_t() const
    {
        return static_cast<std::int16_t>(GetValue());
    }
    EbmlSInteger::operator std::int32_t() const
    {
        return static_cast<std::int32_t>(GetValue());
    }
    EbmlSInteger::operator std::int64_t() const
    {
        return GetValue();
    }
    ...
    */
    filepos_t EbmlSInteger::RenderData(IOCallback& output, bool /* bForceRender */,
    ...
    filepos_t EbmlSInteger::UpdateSize(const ShouldWrite& writeFilter, bool /* bForceRender */)
    {
        ...
        else if (val <= INT64_C(0x7FFFFFFFFF) && val >= INT64_C(-0x8000000000))
        {
            SetSize_(5);
        }
        else if (val <= INT64_C(0x7FFFFFFFFFFF) && val >= INT64_C(-0x800000000000))
        {
            SetSize_(6);
        }
        else if (val <= INT64_C(0x7FFFFFFFFFFFFF) && val >= INT64_C(-0x80000000000000))
        {
            SetSize_(7);
        }
        else
    ...
    {
        if (ReadFully == SCOPE_NO_DATA) return GetSize();
