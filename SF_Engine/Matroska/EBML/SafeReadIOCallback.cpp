...
namespace libebml
{
    ...
    void SafeReadIOCallback::Init(std::unique_ptr<IOCallback> IO)
    {
        mIO = std::move(IO);
    ...
    std::size_t SafeReadIOCallback::GetPosition() const
    {
        return mIO->getFilePointer();
    }

    std::size_t SafeReadIOCallback::GetSize() const
    {
        return mSize;
    }

    std::size_t SafeReadIOCallback::GetRemainingBytes() const
    {
        return GetSize() - GetPosition();
    }

    bool SafeReadIOCallback::IsEmpty() const
    {
        return !GetRemainingBytes();
    }

    std::uint64_t SafeReadIOCallback::GetUIntBE(std::size_t NumBytes)
    {
    ...
    std::uint8_t SafeReadIOCallback::GetUInt8()
    {
        return static_cast<std::uint8_t>(GetUIntBE(1));
    }

    std::uint16_t SafeReadIOCallback::GetUInt16BE()
    {
        return static_cast<std::uint16_t>(GetUIntBE(2));
    }

    std::uint32_t SafeReadIOCallback::GetUInt24BE()
    {
        return static_cast<std::uint32_t>(GetUIntBE(3));
    }

    std::uint32_t SafeReadIOCallback::GetUInt32BE()
    {
        return static_cast<std::uint32_t>(GetUIntBE(4));
    }

    std::uint64_t SafeReadIOCallback::GetUInt64BE()
    {
        return GetUIntBE(8);
    }

    void SafeReadIOCallback::Skip(std::size_t Count)
    {
    ...
    void SafeReadIOCallback::Seek(std::size_t Position)
    {
