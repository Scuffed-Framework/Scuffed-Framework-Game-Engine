...
namespace libebml
{
    ...
    public:
        ...
        // The close callback flushes the file buffers to disk and closes the file. When using the
        // stdio library, this is equivalent to calling fclose. When the close is not successful, an
        ...
        void readStruct(STRUCT& Struct)
        {
            readFully(&Struct, sizeof(Struct));
        }

        void writeFully(const void* Buffer, std::size_t Size);

        template <class STRUCT>
        void writeStruct(const STRUCT& Struct)
        {
            writeFully(&Struct, sizeof(Struct));
        }
    };

    /* cygwin incompatible
    template<class TRAITS>
    std::basic_ostream<char,TRAITS>&operator<<(std::basic_ostream<char,TRAITS>&Stream,seek_mode
    Mode)
    {
      switch(Mode)
      {
