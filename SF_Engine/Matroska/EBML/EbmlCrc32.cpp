...
static constexpr uint32_t CRC32_INDEX(uint32_t c)
{
    return c >> 24;
}
static constexpr uint32_t CRC32_SHIFTED(uint32_t c)
{
    return c << 8;
}
#else
...
namespace libebml
{
    ...
    EbmlCrc32::EbmlCrc32() : EbmlBinary(EbmlCrc32::ClassInfos)
    {
        ResetCRC();
        SetDefaultSize(4);
        SetSize_(4);
    ...
    void EbmlCrc32::AddElementCRC32(EbmlElement& ElementToCRC)
    {
        ...
        if (memSize > std::numeric_limits<std::uint32_t>::max()) return;
    ...
    bool EbmlCrc32::CheckElementCRC32(EbmlElement& ElementToCRC) const
    {
        ...
        const std::uint64_t memSize = memoryBuffer.GetDataBufferSize();
        if (memSize > std::numeric_limits<std::uint32_t>::max()) return false;

        return CheckCRC(m_crc_final, memoryBuffer.GetDataBuffer(),
                        static_cast<std::uint32_t>(memSize));
    }

    filepos_t EbmlCrc32::RenderData(IOCallback& output, bool /* bForceRender */,
    ...
    bool EbmlCrc32::CheckCRC(std::uint32_t inputCRC, const binary* input, std::uint32_t length)
    {
        ...
        while (length >= 4)
        {
            ...
            std::uint32_t chunk{};
            for (int idx = 3; idx >= 0; --idx) chunk = (chunk << 8) | buf[idx];
            crc ^= chunk;
        ...
        while (length--) crc = s_tab.at(CRC32_INDEX(crc) ^ *input++) ^ CRC32_SHIFTED(crc);
