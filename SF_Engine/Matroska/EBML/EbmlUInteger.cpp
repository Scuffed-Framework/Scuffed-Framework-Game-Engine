...
namespace libebml
{
        ...
        : EbmlElementDefaultSameStorage(classInfo, DEFAULT_UINT_SIZE)
    {
        ...
        {
            const auto& def =
                static_cast<const EbmlCallbacksWithDefault<std::uint64_t>&>(classInfo);
            SetValue(def.DefaultValue());
        }
    }

    EbmlUInteger::operator std::uint8_t() const
    {
        return static_cast<std::uint8_t>(GetValue());
    }
    EbmlUInteger::operator std::uint16_t() const
    {
        return static_cast<std::uint16_t>(GetValue());
    }
    EbmlUInteger::operator std::uint32_t() const
    {
        return static_cast<std::uint32_t>(GetValue());
    }
    EbmlUInteger::operator std::uint64_t() const
    {
        return GetValue();
    }
    ...
    */
    filepos_t EbmlUInteger::RenderData(IOCallback& output, bool /* bForceRender */,
