...
namespace libebml
{
    ...
    EbmlVoid::EbmlVoid() : EbmlBinary(EbmlVoid::ClassInfos)
    {
        SetValueIsSet();
    }

    filepos_t EbmlVoid::RenderData(IOCallback& output, bool /* bForceRender */,
    ...
    static void SetVoidSize(EbmlVoid& Elt, const std::uint64_t FullSize)
    {
        ...
        const unsigned int InitialSizeLength =
            CodedSizeLength(FullSize - EBML_ID_LENGTH(Id_EbmlVoid) - 1, 0);
        ...
        if (EBML_ID_LENGTH(Id_EbmlVoid) + NewSizeLength + NewDataLength < FullSize)
        {
            ...
            NewSizeLength = static_cast<decltype(NewSizeLength)>(
                FullSize - (NewDataLength + EBML_ID_LENGTH(Id_EbmlVoid)));
        }
    ...
    std::uint64_t EbmlVoid::ReplaceWith(EbmlElement& EltToReplaceWith, IOCallback& output,
                                        bool ComeBackAfterward, const ShouldWrite& writeFilter)
    {
        EltToReplaceWith.UpdateSize(writeFilter);
        const auto EltSize = EltToReplaceWith.ElementSize(writeFilter);
        if (EltSize == 0) return INVALID_FILEPOS_T;
    ...
    std::uint64_t EbmlVoid::Overwrite(const EbmlElement& EltToVoid, IOCallback& output,
                                      bool ComeBackAfterward, const ShouldWrite& writeFilter)
    {
