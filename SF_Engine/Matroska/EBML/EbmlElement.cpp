...
namespace libebml
{
    ...
    */
                                     ...
                                     std::uint64_t& SizeUnknown)
    {
        ...
        for (unsigned int SizeIdx = 0; SizeIdx < BufferSize && SizeIdx < 8; SizeIdx++)
        {
            if (InBuffer[0] & (SizeBitMask >> SizeIdx))
            {
                ...
                if (PossibleSizeLength > BufferSize) break;
    ...
    EbmlElement::EbmlElement(const EbmlCallbacks& classInfo, std::uint64_t aDefaultSize,
                             bool bValueSet)
        : ClassInfo(classInfo), DefaultSize(aDefaultSize), bValueIsSet(bValueSet)
    {
        Size = DefaultSize;
    }
    ...
    */
    EbmlElement* EbmlElement::FindNextID(IOCallback& DataStream, const EbmlCallbacks& ClassInfos,
                                         std::uint64_t MaxDataSize)
    {
        ...
        while (!bElementFound)
        {
            ...
            if (!bElementFound) return nullptr;
            ...
            do
            {
                ...
                SizeFound = ReadCodedSizeValue(PossibleSize.data(), _SizeLength, SizeUnknown);
            } while (_SizeLength == 0);
        }
        ...
        auto Result = [&]() -> EbmlElement*
        {
            ...
            {
                if (SizeFound == SizeUnknown) return nullptr;
                return new EbmlDummy(pID);
            }
            if (SizeFound != SizeUnknown && MaxDataSize < SizeFound) return nullptr;
            // check if the size is not all 1s
            if (SizeFound == SizeUnknown && !ClassInfos.CanHaveInfiniteSize()) return nullptr;
    ...
    /*!
      ...
      \todo better check of the size checking for upper elements (using a list of size for each
      level)
      \param LowLevel Will be returned with the level of the element found compared to the context
      given
    */
                                              ...
                                              unsigned int MaxLowerLevel)
    {
        ...
        do
        {
            ...
            {
                _SizeLength = PossibleSizeLength;
                SizeFound = ReadCodedSizeValue(&PossibleIdNSize.at(PossibleID_Length), _SizeLength,
                                               SizeUnknown);
                if (_SizeLength != 0)
                {
                    bFound = true;
            ...
            if (bFound)
            {
                ...
                const auto PossibleID =
                    EbmlId(EbmlId::FromBuffer(PossibleIdNSize.data(), PossibleID_Length));
                EbmlElement* Result = CreateElementUsingContext(PossibleID, Context, UpperLevel,
                                                                false, SizeFound == SizeUnknown,
                                                                AllowDummyElt, MaxLowerLevel);
                ...
                if (Result != nullptr)
                {
                    if (AllowDummyElt || !Result->IsDummy())
                    {
                        ...
                        //  + : further parent
                        if (Result->SizeIsValid(SizeFound) &&
                            (SizeFound == SizeUnknown || UpperLevel > 0 || MaxDataSize == 0 ||
                             MaxDataSize >=
                                 (IdStart + PossibleID_Length + _SizeLength + SizeFound)))
                        {
    ...
    */
    EbmlElement* EbmlElement::SkipData(EbmlStream& DataStream, const EbmlSemanticContext& Context,
                                       EbmlElement* TestReadElt, bool AllowDummyElt)
    {
        ...
        if (bSizeIsFinite)
        {
            ...
            DataStream.I_O().setFilePointer(
                SizePosition + CodedSizeLength(Size, SizeLength, bSizeIsFinite) + Size,
                seek_beginning);
        ...
        else
        {
            ...
            while (!bEndFound && Result == nullptr)
            {
                ...
                if (TestReadElt == nullptr)
                {
                    ...
                    Result = DataStream.FindNextElement(Context, bUpperElement, 0xFFFFFFFFL,
                                                        AllowDummyElt);
                }
                else
                {
                ...
                if (Result != nullptr)
                {
                    ...
                    {
                        const auto& MasterContext =
                            static_cast<const EbmlSemanticContextMaster&>(Context);
                        for (EltIndex = 0; EltIndex < EBML_CTX_SIZE(MasterContext); EltIndex++)
                        {
                            if (EbmlId(*Result) == EBML_CTX_IDX_ID(MasterContext, EltIndex))
                            {
                                ...
                                Result = Result->SkipData(
                                    DataStream,
                                    EBML_SEM_CONTEXT(EBML_CTX_IDX(MasterContext, EltIndex)),
                                    nullptr);
    ...
    EbmlElement* EbmlElement::CreateElementUsingContext(const EbmlId& aID,
                                                        ...
                                                        bool AsInfiniteSize, bool bAllowDummy,
                                                        unsigned int MaxLowerLevel)
    {
        EbmlElement* Result = nullptr;
        ...
        if (EBML_CTX_SIZE(Context))
        {
            ...
            for (unsigned int ContextIndex = 0; ContextIndex < EBML_CTX_SIZE(MasterContext);
                 ContextIndex++)
            {
                if (aID == EBML_CTX_IDX_ID(MasterContext, ContextIndex))
                {
                    if (AsInfiniteSize &&
                        !EBML_CTX_IDX_INFO(MasterContext, ContextIndex).CanHaveInfiniteSize())
                        return nullptr;
        ...
        // global elements
        assert(Context.GetGlobalContext !=
               nullptr);  // global should always exist, at least the EBML ones
        ...
        if (tstContext != Context)
        {
            ...
            Result = CreateElementUsingContext(aID, tstContext, LowLevel, true, AsInfiniteSize,
                                               bAllowDummy, MaxLowerLevel);
            if (Result != nullptr)
            {
                return Result;
            }
        ...
        if (EBML_CTX_MASTER(Context) != nullptr && aID == EBML_INFO_ID(*EBML_CTX_MASTER(Context)))
        {
            ...
            if (AsInfiniteSize && !Callbacks.CanHaveInfiniteSize()) return nullptr;
        ...
        if (EBML_CTX_PARENT(Context) != nullptr)
        {
            LowLevel++;
            MaxLowerLevel++;
            return CreateElementUsingContext(aID, *EBML_CTX_PARENT(Context), LowLevel,
                                             IsGlobalContext, AsInfiniteSize, bAllowDummy,
                                             MaxLowerLevel);
        }
    ...
    */
    filepos_t EbmlElement::Render(IOCallback& output, const ShouldWrite& writeFilter,
                                  bool bKeepPosition, bool bForceRender)
    {
        assert(bValueIsSet ||
               CanWrite(writeFilter));  // an element is been rendered without a value set !!!
...
#if !defined(NDEBUG)
        if (static_cast<std::int64_t>(SupposedSize) != (0 - 1)) assert(WrittenSize == SupposedSize);
#endif  // !NDEBUG
        result += WrittenSize;
        return result;
    }
    ...
    */
    filepos_t EbmlElement::RenderHead(IOCallback& output, bool bForceRender,
                                      const ShouldWrite& writeFilter, bool bKeepPosition)
    {
    ...
    {
        if (!CanWrite(writeFilter)) return 0;  // won't be saved
    ...
    {
        if (EbmlId(*A) == EbmlId(*B)) return A->IsSmallerThan(B);

        return false;
    }

    void EbmlElement::Read(EbmlStream& inDataStream, const EbmlSemanticContext& /* Context */,
                           int& /* UpperEltFound */, EbmlElement*& /* FoundElt */,
                           bool /* AllowDummyElt */, ScopeMode ReadFully)
    {
