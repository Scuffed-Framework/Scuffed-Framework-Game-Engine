...
namespace libebml
{
    ...
    MemReadIOCallback::MemReadIOCallback(void const* Ptr, std::size_t Size)
    {
        Init(Ptr, Size);
    }
    ...
    void MemReadIOCallback::Init(void const* Ptr, std::size_t Size)
    {
    ...
    std::size_t MemReadIOCallback::read(void* Buffer, std::size_t Size)
    {
    ...
    void MemReadIOCallback::setFilePointer(std::int64_t Offset, seek_mode Mode)
    {
        std::int64_t NewPosition = Mode == seek_beginning ? Offset
                                   : Mode == seek_end
                                       ? static_cast<std::int64_t>(mEnd - mStart) + Offset
