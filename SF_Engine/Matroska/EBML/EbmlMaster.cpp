...
namespace libebml
{
          ...
          Checksum(ElementToClone.Checksum)
    {
        ...
        for (const auto& e : ElementToClone.ElementList) ElementList.push_back(e->Clone());
    }
    ...
    {
        assert(i < GetSize());
        if (i < GetSize()) return MyTable[i];

        std::stringstream ss;
        ss << "EbmlSemanticContext::GetSemantic: programming error: index i outside of table size ("
           << i << " >= " << GetSize() << ")";
    ...
    */
    filepos_t EbmlMaster::RenderData(IOCallback& output, bool bForceRender,
                                     const ShouldWrite& writeFilter)
    {
        ...
        {  // old school
            for (auto Element : ElementList)
            {
                if (!Element->CanWrite(writeFilter)) continue;
        ...
        {  // new school
            ...
            while (memSize != 0)
            {
                const auto fillSize = static_cast<std::uint32_t>(
                    std::min<std::uint64_t>(std::numeric_limits<std::uint32_t>::max(), memSize));
    ...
    */
    ...
    {
        if (!CanWrite(writeFilter)) return 0;

        SetSize_(0);

        if (!IsFiniteSize()) return std::numeric_limits<std::uint64_t>::max();
    ...
    filepos_t EbmlMaster::WriteHead(IOCallback& output, unsigned int nSizeLength,
                                    const ShouldWrite& writeFilter)
    {
    ...
    bool EbmlMaster::ProcessMandatory()
    {
        ...
        for (unsigned int EltIdx = 0; EltIdx < EBML_CTX_SIZE(MasterContext); EltIdx++)
        {
            if (EBML_CTX_IDX(MasterContext, EltIdx).IsMandatory() &&
                EBML_CTX_IDX(MasterContext, EltIdx).IsUnique())
            {
                //      assert(EBML_CTX_IDX(MasterContext,EltIdx).Create != NULL);
                if (PushElement(EBML_SEM_CREATE(EBML_CTX_IDX(MasterContext, EltIdx)))) continue;
...
//         const char * MissingName = semcb.GetName();
    ...
    {
        auto it =
            std::find_if(ElementList.begin(), ElementList.end(), [&](const EbmlElement* Element)
    ...
    */
    ...
    EbmlElement* EbmlMaster::FindNextElt(const EbmlElement& PastElt) const
    {
        ...
        {
            it = std::find_if(it + 1, ElementList.end(),
                              [&](auto&& element) { return EbmlId(PastElt) == EbmlId(*element); });

            if (it != ElementList.end()) return *it;
        }
    ...
    EbmlElement* EbmlMaster::AddNewElt(const EbmlCallbacks& Callbacks)
    {
        ...
        if (NewElt == nullptr) return nullptr;
    ...
    */
    void EbmlMaster::Read(EbmlStream& inDataStream, const EbmlSemanticContext& sContext,
                          int& UpperEltFound, EbmlElement*& FoundElt, bool AllowDummyElt,
                          ScopeMode ReadFully)
    {
        if (ReadFully == SCOPE_NO_DATA) return;
        ...
        if (MaxSizeToRead > 0)
        {
            ...
            ElementLevelA =
                inDataStream.FindNextElement(sContext, UpperEltFound, MaxSizeToRead, AllowDummyElt);
            while (ElementLevelA != nullptr && UpperEltFound <= 0 && MaxSizeToRead > 0)
            {
                if (IsFiniteSize() && ElementLevelA->IsFiniteSize())
                    MaxSizeToRead =
                        GetEndPosition() -
                        ElementLevelA->GetEndPosition();  // even if it's the default value
                ...
                else
                {
                    try
                    {
                        ElementLevelA->Read(inDataStream, EBML_CONTEXT(ElementLevelA),
                                            UpperEltFound, FoundElt, AllowDummyElt, ReadFully);
                    }
                    catch (...)
                    {
                    ...
                    if (ElementLevelA->IsFiniteSize())
                    {
                        ...
                        if (DeleteElement) delete ElementLevelA;
                    }
                    else
                    {
                        if (DeleteElement) delete ElementLevelA;
                        ...
                        {
                            --UpperEltFound;
                            if (UpperEltFound > 0 || MaxSizeToRead <= 0) goto processCrc;
                ...
                {
                    UpperEltFound++;
                    if (UpperEltFound < 0) goto processCrc;
                }

                if (MaxSizeToRead <= 0) goto processCrc;  // this level is finished

                ElementLevelA = inDataStream.FindNextElement(sContext, UpperEltFound, MaxSizeToRead,
                                                             AllowDummyElt);
            }
            if (UpperEltFound > 0)
            {
    ...
    processCrc:
        ...
        auto CrcItr = std::find_if(ElementList.begin(), ElementList.end(), [](auto&& element)
    ...
    {
        if ((ElementList.empty()) && position) return false;
    ...
    bool EbmlMaster::InsertElement(EbmlElement& element, const EbmlElement& before)
    {
        ...
        if (Itr == ElementList.end()) return false;
