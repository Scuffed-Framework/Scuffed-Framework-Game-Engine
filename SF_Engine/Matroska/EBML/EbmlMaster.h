    ...
    */
    ...
    public:
        ...
        */
        ...
        filepos_t UpdateSize(const ShouldWrite& writeFilter = WriteSkipDefault,
                             bool bForceRender = false) override;

        bool PushElement(EbmlElement& element);
        std::uint64_t GetSize() const override
        {
            if (IsFiniteSize()) return EbmlElement::GetSize();
            return (0 - 1);
        }

        /*!
          \brief find the first element corresponding to the EBML class of the element, NULL if not
          found
        */
        ...
        */
        void Read(EbmlStream& inDataStream, const EbmlSemanticContext& Context, int& UpperEltFound,
                  EbmlElement*& FoundElt, bool AllowDummyElt,
                  ScopeMode ReadFully = SCOPE_ALL_DATA) override;
        ...
        */
        ...
        std::vector<EbmlElement*> const& GetElementList() const
        {
            return ElementList;
        }
        std::vector<EbmlElement*>& GetElementList()
        {
            return ElementList;
        }

        inline EBML_MASTER_ITERATOR begin()
        {
            return ElementList.begin();
        }
        inline EBML_MASTER_ITERATOR end()
        {
            return ElementList.end();
        }
        inline EBML_MASTER_RITERATOR rbegin()
        {
            return ElementList.rbegin();
        }
        inline EBML_MASTER_RITERATOR rend()
        {
            return ElementList.rend();
        }
        inline EBML_MASTER_CONST_ITERATOR begin() const
        {
            return ElementList.begin();
        }
        inline EBML_MASTER_CONST_ITERATOR end() const
        {
            return ElementList.end();
        }
        inline EBML_MASTER_CONST_RITERATOR rbegin() const
        {
            return ElementList.rbegin();
        }
        inline EBML_MASTER_CONST_RITERATOR rend() const
        {
            return ElementList.rend();
        }
        ...
        */
        filepos_t WriteHead(IOCallback& output, unsigned int SizeLength,
                            const ShouldWrite& writeFilter = WriteSkipDefault);

        void EnableChecksum(bool bIsEnabled = true)
        {
            bChecksumUsed = bIsEnabled;
        }
        bool HasChecksum() const
        {
            return bChecksumUsed;
        }
        bool VerifyChecksum() const;
        std::uint32_t GetCrc32() const
        {
            return Checksum.GetCrc32();
        }
    ...
    static inline constexpr const EbmlSemanticContextMaster& tEBML_INFO_CONTEXT(
        const EbmlCallbacksMaster& cb)
    {
