...
namespace SF::Engine
{
    ...
    public:
        ...
        explicit Crc32(const void *data, size_t size, bool forceLowerCase = false) : Crc32(reinterpret_cast<const uint8 *>(data), size, forceLowerCase) {}

        template <class ByteType, class = enable_if_t<sizeof(ByteType) == 1>>
        constexpr Crc32(const ByteType *data, size_t size, bool forceLowerCase = false) { Set(data, size, forceLowerCase); }

        explicit constexpr Crc32(std::span<const byte> inputSpan)
        {
        ...
        [[nodiscard]] constexpr uint32 GetValue() const
        {
            return ~crc;
        }
